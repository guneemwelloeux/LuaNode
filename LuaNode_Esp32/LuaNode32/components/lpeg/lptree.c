...
*/
static TTree *newemptycap(lua_State *L, int cap) { return auxemptycap(newtree(L, 2), cap); }
...
static void getfirstrule(lua_State *L, int arg, int postab)
{
    ...
    if (lua_isstring(L, -1))
    {                         /* is it the name of initial rule? */
...
static int collectrules(lua_State *L, int arg, int *totalsize)
{
    ...
    {
        if (lua_tonumber(L, -2) == 1 || lp_equal(L, -2, postab + 1))
        {                  /* initial rule? */
...
static struct luaL_Reg metareg[] = {{"__mul", lp_seq}, {"__add", lp_choice}, {"__pow", lp_star},
