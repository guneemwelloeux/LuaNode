...
static const net_table_name NET_TABLES[] = {"net.tcpserver", "net.tcpsocket", "net.udpsocket"};
...
int net_mycall(lua_State *L)
{
    if (myref != LUA_NOREF)
    {
        lua_rawgeti(L, LUA_REGISTRYINDEX, myref);
        lua_pcall(L, 0, 0, 0);
    }
    printf("myref = 0\n");
    return 0;
...
int net_myregistrer(lua_State *L)
{
    int argc = lua_gettop(L);
    lua_pushvalue(L, 1);
    myref = luaL_ref(L, LUA_REGISTRYINDEX);
    return 0;
}
...
int lwip_lua_checkerr(lua_State *L, err_t err)
{
    switch (err)
    {
        ...
        case ERR_RTE:
            return luaL_error(L, "routing problem");
        case ERR_INPROGRESS:
            return luaL_error(L, "in progress");
        case ERR_VAL:
            return luaL_error(L, "illegal value");
        case ERR_WOULDBLOCK:
            return luaL_error(L, "would block");
        case ERR_ABRT:
            return luaL_error(L, "connection aborted");
        case ERR_RST:
            return luaL_error(L, "connection reset");
        case ERR_CLSD:
            return luaL_error(L, "connection closed");
        case ERR_CONN:
            return luaL_error(L, "not connected");
        case ERR_ARG:
            return luaL_error(L, "illegal argument");
        case ERR_USE:
            return luaL_error(L, "address in use");
        case ERR_IF:
            return luaL_error(L, "netif error");
        case ERR_ISCONN:
...
static void net_err_cb(void *arg, err_t err)
{
    ...
    if (!ud || ud->type != TYPE_TCP_CLIENT || ud->self_ref == LUA_NOREF)
        return;
...
static err_t net_sent_cb(void *arg, struct tcp_pcb *tpcb, u16_t len)
{
    ...
    post_net_sent(ud);
    // TODO: if we can't post this, we effectively stall this socket, how to
    // fix?
    return ERR_OK;
}
...
{
    lnet_userdata *ud = (lnet_userdata *)arg;
    if (!ud || ud->type != TYPE_TCP_SERVER || !ud->pcb)
        return ERR_ABRT;
    if (ud->self_ref == LUA_NOREF || ud->server.cb_accept_ref == LUA_NOREF)
        return ERR_ABRT;
...
int net_on(lua_State *L)
{
    ...
    switch (ud->type)
    {
        case TYPE_TCP_CLIENT:
            ...
            {
                refptr = &ud->client.cb_connect_ref;
                break;
            }
            if (strcmp("disconnection", name) == 0)
            {
                refptr = &ud->client.cb_disconnect_ref;
                break;
            }
            if (strcmp("reconnection", name) == 0)
            {
                refptr = &ud->client.cb_reconnect_ref;
                break;
            }
        case TYPE_UDP_SOCKET:
            if (strcmp("dns", name) == 0)
            {
                refptr = &ud->client.cb_dns_ref;
                break;
            }
            if (strcmp("receive", name) == 0)
            {
                refptr = &ud->client.cb_receive_ref;
                break;
            }
            if (strcmp("sent", name) == 0)
            {
                refptr = &ud->client.cb_sent_ref;
                break;
            }
            break;
        default:
...
int net_send(lua_State *L)
{
    ...
    if (ud->type == TYPE_UDP_SOCKET)
    {
        ...
        if (port == 0)
            return luaL_error(L, "need port");
        const char *domain = luaL_checklstring(L, stack++, &dl);
        if (!domain)
            return luaL_error(L, "need IP address");
        if (!ipaddr_aton(domain, &addr))
            return luaL_error(L, "invalid IP address");
    }
...
int net_close(lua_State *L)
{
    ...
    if (ud->type == TYPE_TCP_SERVER || (ud->pcb == NULL && ud->client.wait_dns == 0))
    {
...
int net_delete(lua_State *L)
{
    lnet_userdata *ud = net_get_udata(L);
    if (!ud)
        return luaL_error(L, "no user data");
    if (ud->pcb)
    {
        switch (ud->type)
        {
...
static int net_multicastJoinLeave(lua_State *L, int join)
{
    ...
    if (if_ip == NULL || il > 15 || il < 7)
        return luaL_error(L, "invalid if ip");
    ip4addr_aton(if_ip, &if_addr);

    if (!lua_isstring(L, 2))
        return luaL_error(L, "wrong arg type");
    multicast_ip = luaL_checklstring(L, 2, &il);
    if (multicast_ip == NULL || il > 15 || il < 7)
        return luaL_error(L, "invalid multicast ip");
...
static int net_multicastJoin(lua_State *L) { return net_multicastJoinLeave(L, 1); }

// Lua: net.multicastLeave(ifip, multicastip)
// if ifip "" or "any" all interfaces are affected
static int net_multicastLeave(lua_State *L) { return net_multicastJoinLeave(L, 0); }
...
static int net_dns_static(lua_State *L)
{
    ...
    err_t err = dns_gethostbyname(domain, &ev->resolved_ip, net_dns_static_cb, ev);
    if (err == ERR_OK)
    {
...
static int net_setdnsserver(lua_State *L)
{
    ...
    if (l > 16 || server == NULL || !ipaddr_aton(server, &ipaddr) || ip_addr_isany(&ipaddr))
...
static void lerr_cb(lua_State *L, lnet_userdata *ud, err_t err)
{
    ...
    if (err != ERR_OK && ud->client.cb_reconnect_ref != LUA_NOREF)
        ...
        ref = ud->client.cb_disconnect_ref;
    if (ref != LUA_NOREF)
    {
...
static void handle_net_event(task_param_t param, task_prio_t prio)
{
    ...
    switch (ev->event)
    {
        ...
        case DNSSTATIC:
            ...
            break;
        case CONNECTED:
            lconnected_cb(L, ev->ud);
            break;
        case ACCEPT:
            laccept_cb(L, ev->ud, ev->accept_newpcb);
            break;
        case RECVDATA:
            lrecv_cb(L, ev->ud, &ev->recvdata);
            break;
        case SENTDATA:
            lsent_cb(L, ev->ud);
            break;
        case ERR:
            lerr_cb(L, ev->ud, ev->err);
            break;
    }
...
static const LUA_REG_TYPE net_tcpserver_map[] = {
    {LSTRKEY("listen"), LFUNCVAL(net_listen)},       {LSTRKEY("getaddr"), LFUNCVAL(net_getaddr)},
    {LSTRKEY("close"), LFUNCVAL(net_close)},         {LSTRKEY("__gc"), LFUNCVAL(net_delete)},
    {LSTRKEY("__index"), LROVAL(net_tcpserver_map)}, {LNILKEY, LNILVAL}};

static const LUA_REG_TYPE net_tcpsocket_map[] = {{LSTRKEY("connect"), LFUNCVAL(net_connect)},
...
static const LUA_REG_TYPE net_udpsocket_map[] = {{LSTRKEY("listen"), LFUNCVAL(net_listen)},
                                                 {LSTRKEY("close"), LFUNCVAL(net_close)},
                                                 {LSTRKEY("on"), LFUNCVAL(net_on)},
                                                 {LSTRKEY("send"), LFUNCVAL(net_send)},
                                                 {LSTRKEY("dns"), LFUNCVAL(net_dns)},
...
static const LUA_REG_TYPE net_dns_map[] = {{LSTRKEY("setdnsserver"), LFUNCVAL(net_setdnsserver)},
                                           {LSTRKEY("getdnsserver"), LFUNCVAL(net_getdnsserver)},
                                           {LSTRKEY("resolve"), LFUNCVAL(net_dns_static)},
                                           {LNILKEY, LNILVAL}};

const LUA_REG_TYPE net_map[] = {{LSTRKEY("createServer"), LFUNCVAL(net_createServer)},
                                ...
                                {LSTRKEY("mycall"), LFUNCVAL(net_mycall)},
                                {LSTRKEY("myregister"), LFUNCVAL(net_myregistrer)},
