...
ETSEvent espconn_TaskQueue[espconn_TaskQueueLen];

static err_t espconn_client_recv(void *arg, struct tcp_pcb *pcb, struct pbuf *p, err_t err);
static void espconn_client_close(void *arg, struct tcp_pcb *pcb, uint8 type);

static err_t espconn_server_recv(void *arg, struct tcp_pcb *pcb, struct pbuf *p, err_t err);
static void espconn_server_close(void *arg, struct tcp_pcb *pcb, uint8 type);
...
struct tcp_pcb *ICACHE_FLASH_ATTR espconn_find_current_pcb(espconn_msg *pcurrent_msg)
{
    ...
    if (pcurrent_msg->preverse == NULL)
    { /*Find the server's TCP block*/
        if (local_ip == 0 || local_port == 0)
            return pcurrent_msg->pcommon.pcb;

        for (find_pcb = tcp_active_pcbs; find_pcb != NULL; find_pcb = find_pcb->next)
        {
            if ((find_pcb->remote_port == remote_port) && (find_pcb->remote_ip.u_addr.ip4.addr == remote_ip) &&
                (find_pcb->local_port == local_port) && (find_pcb->local_ip.u_addr.ip4.addr == local_ip))
                return find_pcb;
        }
...
static void ICACHE_FLASH_ATTR espconn_tcp_reconnect(void *arg)
{
    ...
    if (precon_cb != NULL)
    {
        ...
        {
            if (espconn != NULL)
            { /*Process the server's message block*/
            ...
            { /*Process the client's message block*/
                ...
                if (espconn_reuse_disabled(pdiscon_cb))
                {
                    ...
                    while (cpcb != NULL)
                    {
                        ...
                        if (pcb_remove)
                        {
                            ...
                            {
                                LWIP_ASSERT(
                                    "espconn_tcp_delete: middle cpcb != "
                                    "tcp_tw_pcbs",
                                    cpcb != tcp_tw_pcbs);
                                prev->next = cpcb->next;
                            }
...
// espconn_Task(void *pvParameters)
// {
// 	espcon_event_t *events = (espcon_event_t *) pvParameters;
...
// 				/*remove the node from the client's active
// connection
// list*/ 				espconn_list_delete(&plink_active,
// task_msg); 				espconn_tcp_reconnect(task_msg); 				break; 			case
// SIG_ESPCONN_CLOSE:
// 				/*remove the node from the client's active
// connection
// list*/ 				espconn_list_delete(&plink_active,
// task_msg); 				espconn_tcp_disconnect_successful(task_msg); 				break;
// default: 				break;
// 		}
// 	}
// }
static void ICACHE_FLASH_ATTR espconn_Task(ETSEvent *events)
{
    espconn_msg *task_msg = NULL;
    struct espconn *pespconn = NULL;
...
{
    espconn_msg *ptcp_sent = (espconn_msg *)arg;
    struct tcp_pcb *pcb = NULL;
    err_t err = 0;
    u16_t len = 0;
    ...
    /*Find out what we can send and send it, offset the buffer point for next
     * send*/
    if (err == ERR_OK)
    {
 ...
 * Returns      : none
 *******************************************************************************/
static void ICACHE_FLASH_ATTR espconn_client_close(void *arg, struct tcp_pcb *pcb, uint8 type)
{
    err_t err;
...
{
    // 1st, according to espconn code, have to find out the escpconn_msg by
    // pespconn;
    ...
    if (value != true)
    {
        printf("RecvHold, By pespconn,find conn_msg fail\n");
        return ESPCONN_ARG;
    }
 ...
 *                p -- The received data (or NULL when the connection has been
 *closed!) err -- An error code if there has been an error receiving Returns :
 ...
 *                ERR_ABRT: if you have called tcp_abort from within the
 *function!
 *******************************************************************************/
...
static err_t ICACHE_FLASH_ATTR espconn_client_connect(void *arg, struct tcp_pcb *tpcb, err_t err)
{
    ...
    if (err == ERR_OK)
    {
        ...
        /*Default set the total number of espconn_buf on the unsent lists for
         * one*/
...
sint8 ICACHE_FLASH_ATTR espconn_tcp_client(struct espconn *espconn)
{
    ...
    else
    {
        ...
        espconn_list_creat(&plink_active,
                           pclient);  // printf("espconn_msg 3: %p\n", pclient);
        ...
        pclient->pcommon.err =
            tcp_connect(pcb, &ipaddr, pclient->pespconn->proto.tcp->remote_port, espconn_client_connect);
 ...
 * Returns      : none
 *******************************************************************************/
static void ICACHE_FLASH_ATTR espconn_server_close(void *arg, struct tcp_pcb *pcb, uint8 type)
{
    err_t err;
 ...
 * If there has been no data sent (which resets the retries) in 3 seconds,
 *close. If the last portion of a file has not been sent in 3 seconds, close.
 *
 * This could be increased, but we don't want to waste resources for bad
 *connections. Parameters   : arg -- Additional argument to pass to the callback
 *function pcb -- The connection pcb for which data has been acknowledged
 * Returns      : ERR_OK: try to send some data by calling tcp_output
 *                ERR_ABRT: if you have called tcp_abort from within the
 *function!
 *******************************************************************************/
static err_t ICACHE_FLASH_ATTR espconn_server_poll(void *arg, struct tcp_pcb *pcb)
{
    ...
    if (pcb->state == ESTABLISHED)
    {
        ...
        { /*no data sent in one active connection's set timeout, close.*/
        ...
        else
        {
            ...
            while (ptime_msg != NULL)
            {
                ...
                {
                    if (ptime_msg->pcommon.timeout != 0)
                    { /*no data sent in server's set timeout, close.*/
...
static void ICACHE_FLASH_ATTR esponn_server_err(void *arg, err_t err)
{
    ...
    if (pserr_cb != NULL)
    {
        ...
        //		/*remove the node from the server's active connection
        // list*/ 		espconn_list_delete(&plink_active, pserr_cb);
...
static err_t ICACHE_FLASH_ATTR espconn_tcp_accept(void *arg, struct tcp_pcb *pcb, err_t err)
{
    ...
    /*Ensure the active connection is less than the count of active connections
     * on the server*/
...
void ICACHE_FLASH_ATTR espconn_init(void)
{
    ...
    ets_run();
    // espcon_event_t even;
    // xTaskCreatePinnedToCore(&espconn_Task, "espconn_Task", 4096, &even, 5,
    // NULL, 0);
}
