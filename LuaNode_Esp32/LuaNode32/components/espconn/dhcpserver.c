...
static sint16_t ICACHE_FLASH_ATTR parse_msg(struct dhcps_msg *m, u16_t len)
{
    if (memcmp((char *)m->options, &magic_cookie, sizeof(magic_cookie)) == 0)
    {
        ...
        //	             * �϶��ǵ�һ��ʹ��
        //	            */
        //	            memcpy((char *)old_xid, (char *)m->xid,
        // sizeof(m->xid)); 	        }else{
        //	            /*
        //	             * ���δ����DHCP msg��Я���xid���ϴμ�¼�Ĳ�ͬ��
        //	             * �϶�Ϊ��ͬ��DHCP�ͻ��˷��ͣ���ʱ����Ҫ����Ŀͻ���IP
        //	             * ���� 192.168.4.100(0x6404A8C0) <-->
        // 192.168.4.200(0xC804A8C0)
        //	             *
        //	            */
        //	            if(memcmp((char *)old_xid, (char *)m->xid,
        // sizeof(m->xid)) != 0){
        /*
         * ��¼���ε�xid�ţ�ͬʱ�����IP����
         */
        ...
        renew = false;
        //							addr_tmp.addr =
        // htonl(client_address_plus.addr);
        // addr_tmp.addr++; 							client_address_plus.addr =
        // htonl(addr_tmp.addr);
 ...
 */
///////////////////////////////////////////////////////////////////////////////////
static void ICACHE_FLASH_ATTR handle_dhcp(void *arg, struct udp_pcb *pcb, struct pbuf *p, const ip_addr_t *addr,
                                          uint16_t port)
{
...
static void ICACHE_FLASH_ATTR wifi_softap_init_dhcps_lease(uint32 ip)
{
    ...
    if (dhcps_lease.enable == true)
    {
        ...
        {
            /*config ip information must be in the same segment as the local
             * ip*/
            softap_ip >>= 8;
...
#if DHCPS_DEBUG
    printf(
        "dhcps:dhcps_start->udp_recv function Set a receive callback "
...
bool ICACHE_FLASH_ATTR wifi_softap_get_dhcps_lease(struct dhcps_lease *please)
{
    ...
    else
    {
        ...
        //		if (wifi_softap_dhcps_status() == DHCP_STOPPED){
        //			please->start_ip.addr =
        // htonl(dhcps_lease.start_ip.addr); 			please->end_ip.addr
        // = htonl(dhcps_lease.end_ip.addr);
        //		}
    }
