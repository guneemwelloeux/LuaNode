...
static bool is_cache_flash_addr(uint32_t addr) { return addr >= 0x3F400000 && addr < 0x3FC00000; }
...
uint32_t platform_flash_get_num_sectors(void) { return flash_safe_get_sec_num(); }
...
uint32_t platform_flash_write(const void *from, uint32_t toaddr, uint32_t size)
{
    ...
    if (toaddr & blkmask)
    {
        ...
        for (i = rest; size && (i < blksize); i++, size--, pfrom++) tmpdata[i] = *pfrom;
    ...
    if (rest)
    {
        ...
        for (i = 0; size && (i < rest); i++, size--, pfrom++) tmpdata[i] = *pfrom;
...
uint32_t platform_flash_read(void *to, uint32_t fromaddr, uint32_t size)
{
    ...
    if (fromaddr & blkmask)
    {
        ...
        platform_s_flash_read(tmpdata, temp, blksize);
        for (i = rest; size && (i < blksize); i++, size--, pto++) *pto = tmpdata[i];
    ...
    if (rest)
    {
        ...
        for (i = 0; size && (i < rest); i++, size--, pto++) *pto = tmpdata[i];
    }
    return ssize;
