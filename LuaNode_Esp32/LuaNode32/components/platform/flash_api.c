...
static inline esp_image_header_t flash_load_rom_header(void)
{
    ...
    if (ESP_OK != spi_flash_read(FLASH_HDR_ADDR, (uint32_t *)&hdr, sizeof(hdr)))
    {
...
#define DETECT_SZ 32
    ...
    if (ESP_OK == spi_flash_read(offs, (uint32_t *)data_orig, DETECT_SZ))
    {
        ...
        while ((detected_size < FLASH_SIZE_16MBYTE) &&
               (ESP_OK == spi_flash_read(detected_size + offs, (uint32_t *)data_new, DETECT_SZ)) &&
...
uint16_t flash_safe_get_sec_num(void) { return (flash_safe_get_size_byte() / (SPI_FLASH_SEC_SIZE)); }
    ...
    if (flash_size == 0)
    {
        switch (flash_load_rom_header().spi_size)
        {
            ...
            case ESP_IMAGE_FLASH_SIZE_1MB:
                flash_size = FLASH_SIZE_1MBYTE;
                break;
            case ESP_IMAGE_FLASH_SIZE_2MB:
                flash_size = FLASH_SIZE_2MBYTE;
                break;
            case ESP_IMAGE_FLASH_SIZE_4MB:
                flash_size = FLASH_SIZE_4MBYTE;
                break;
            case ESP_IMAGE_FLASH_SIZE_8MB:
                flash_size = FLASH_SIZE_8MBYTE;
                break;
            case ESP_IMAGE_FLASH_SIZE_16MB:
                flash_size = FLASH_SIZE_16MBYTE;
                break;
        }
    }
    return flash_size;
...
bool flash_rom_set_size_byte(uint32_t size)
{
    ...
    switch (size)
    {
        case FLASH_SIZE_1MBYTE:
            size_code = ESP_IMAGE_FLASH_SIZE_1MB;
            break;
        case FLASH_SIZE_2MBYTE:
            size_code = ESP_IMAGE_FLASH_SIZE_2MB;
            break;
        case FLASH_SIZE_4MBYTE:
            size_code = ESP_IMAGE_FLASH_SIZE_4MB;
            break;
        case FLASH_SIZE_8MBYTE:
            size_code = ESP_IMAGE_FLASH_SIZE_8MB;
            break;
        case FLASH_SIZE_16MBYTE:
            size_code = ESP_IMAGE_FLASH_SIZE_16MB;
            break;
        default:
            ok = false;
            break;
...
uint16_t flash_rom_get_sec_num(void) { return (flash_rom_get_size_byte() / (SPI_FLASH_SEC_SIZE)); }

uint8_t flash_rom_get_mode(void) { return flash_load_rom_header().spi_mode; }
...
uint32_t flash_rom_get_speed(void)
{
    switch (flash_load_rom_header().spi_speed)
    {
        ...
        case ESP_IMAGE_SPI_SPEED_26M:
            return 26700000;  // TODO: verify 26.7MHz
