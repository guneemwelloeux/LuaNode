/**
  This file encapsulates the SDK-based task handling for the NodeMCU Lua
  firmware.
 */
#include "task/task.h"
...
task_handle_t task_get_id(task_callback_t t)
{
    ...
    {
        /* With a brick size of 4 this branch is taken at 0, 4, 8 ... and the
         * new size is +4 */
        task_func = (task_callback_t *)realloc(task_func,
...
{
    if (priority >= TASK_PRIORITY_COUNT || !task_Q[priority] || (handle & TASK_HANDLE_MASK) != TASK_HANDLE_MONIKER)
        return false;
...
static void dispatch(task_event_t *e, uint8_t prio)
{
    ...
    if ((handle & TASK_HANDLE_MASK) == TASK_HANDLE_MONIKER)
    {
        ...
        {
            /* call the registered task handler with the specified parameter and
             * priority */
