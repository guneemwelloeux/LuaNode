...
void *luaR_findglobal(const char *name, unsigned len)
{
    ...
    for (i = 0; lua_rotable[i].name; i++)
        if (*lua_rotable[i].name != '\0' && strlen(lua_rotable[i].name) == len &&
            !strncmp(lua_rotable[i].name, name, len))
        {
...
static void luaR_next_helper(lua_State *L, const luaR_entry *pentries, int pos, TValue *key, TValue *val)
{
    ...
    if (pentries[pos].key.type != LUA_TNIL)
    {
        ...
        if (pentries[pos].key.type == LUA_TSTRING)
            setsvalue(L, key, luaS_newro(L, pentries[pos].key.id.strkey)) else setnvalue(
                key, (lua_Number)pentries[pos].key.id.numkey) setobj2s(L, val, &pentries[pos].value);
    }
}
...
int luaR_isrotable(void *p) { return RODATA_START_ADDRESS <= (char *)p && (char *)p <= RODATA_END_ADDRESS; }
#endif
