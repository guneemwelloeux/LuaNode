...
static void DumpIntWithSize(int x, int sizeof_int, DumpState* D)
{
    ...
    switch (sizeof_int)
    {
        ...
        {
            if (x > 0x7F || x < (-0x80))
                D->status = LUA_ERR_CC_INTOVERFLOW;
            DumpChar(x, D);
        }
        break;
        case 2:
...
static void DumpInt(int x, DumpState* D) { DumpIntWithSize(x, D->target.sizeof_int, D); }
...
static void DumpNumber(lua_Number x, DumpState* D)
{
    ...
    {
        if (((float)(int)x) != x)
            D->status = LUA_ERR_CC_NOTINTEGER;
        DumpIntWithSize(x, D->target.sizeof_lua_Number, D);
    }
    else
    {
        switch (D->target.sizeof_lua_Number)
        {
            ...
            case 8:
            {
                ...
                // the memory representation. This is the default compilation
                // mode for ARM targets with non-EABI gcc
