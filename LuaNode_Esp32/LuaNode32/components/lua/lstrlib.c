...
static int str_gsub(lua_State *L)
{
    ...
    luaL_argcheck(
        L,
        tr == LUA_TNUMBER || tr == LUA_TSTRING || tr == LUA_TFUNCTION || tr == LUA_TTABLE || tr == LUA_TLIGHTFUNCTION,
        3, "string/function/table/lightfunction expected");
    luaL_buffinit(L, &b);
    ms.L = L;
    ms.src_init = src;
    ms.src_end = src + srcl;
...
#include "lrodefs.h"
const LUA_REG_TYPE strlib[] = {{LSTRKEY("byte"), LFUNCVAL(str_byte)},
