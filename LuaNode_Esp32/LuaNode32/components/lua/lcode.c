    ...
    switch (e->k)
    {
        ...
        case VNIL:
        {
            ...
            { /* constant fit in RK operand? */
                e->u.s.info = (e->k == VNIL)
                                  ? nilK(fs)
                                  : (e->k == VKNUM) ? luaK_numberK(fs, e->u.nval) : boolK(fs, (e->k == VTRUE));
                e->k = VK;
...
static unsigned char *growLineInfo(FuncState *fs)
{
    ...
    luaM_growvector(fs->L, f->packedlineinfo, fs->packedlineinfoSize, fs->packedlineinfoSize, unsigned char, MAX_INT,
                    "code size overflow");
...
{
#ifdef LUA_OPTIMIZE_DEBUG
    /* The fixup line can be the same as existing one and in this case there's
     * nothing to do */
    if (line != fs->lastline)
    {
        ...
        if (*p > 1)
        {
            (*p)--; /* this is simply decrementing the last count a multi-PC
                       line */
        }
        else
        {
            /* it's a bit more complicated if it's the 1st instruction on the
             * line */
            int delta = 0;
            unsigned char code;
            ...
            while ((code = *p) & INFO_DELTA_MASK)
            {
                *p-- = INFO_FILL_BYTE;
                ...
                else
                {
                    ...
                    if (code & INFO_SIGN_MASK)
                        delta = -delta;
                }
            }
            /* and reposition the FuncState lastline pointers at the previous
             * instruction count */
            fs->lastline -= delta + 1;
...
static int luaK_code(FuncState *fs, Instruction i, int line)
{
    ...
    {
        /* The last instruction is occasionally overwritten as part of branch
         * optimisation*/
