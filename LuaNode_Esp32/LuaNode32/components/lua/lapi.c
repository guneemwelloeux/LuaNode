...
LUA_API void lua_xmove(lua_State *from, lua_State *to, int n)
{
    ...
    from->top -= n;
    for (i = 0; i < n; i++) { setobj2s(to, to->top++, from->top + i); }
    lua_unlock(to);
}
...
LUA_API void lua_rawgeti(lua_State *L, int idx, int n)
{
    ...
    setobj2s(L, L->top, ttistable(o) ? luaH_getnum(hvalue(o), n) : luaH_getnum_ro(rvalue(o), n)) api_incr_top(L);
    lua_unlock(L);
}
...
LUA_API int lua_getmetatable(lua_State *L, int objindex)
{
    ...
    else
    {
        if (luaR_isrotable(mt))
            setrvalue(L->top, mt) else sethvalue(L, L->top, mt) api_incr_top(L);
        res = 1;
    }
    lua_unlock(L);
    return res;
...
LUA_API int lua_gc(lua_State *L, int what, int data)
{
    ...
    switch (what)
    {
        ...
        case LUA_GCSTEP:
        {
            if (is_block_gc(L))
            {
                res = 1; /* gc is block so we need to pretend that the
