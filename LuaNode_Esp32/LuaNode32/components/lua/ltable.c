...
** first, check whether the moving node's main position is free. If not, check
*whether
...
** (colliding node is in its main position), moving node goes to an empty
*position.
*/
static int move_node(lua_State *L, Table *t, Node *node)
{
    ...
    if (find_prev_node(mp, node) != NULL)
        return 1;
    ...
    if (!ttisnil(gval(mp)) || mp == dummynode)
    {
        ...
        { /* is colliding node out of its main position? */
            ...
            if (prev != NULL)
                gnext(prev) = node; /* redo the chain with `n' in place of `mp' */
            *node = *mp;            /* copy colliding node into free pos. (mp->next also
                                       goes) */
            *mp = tmp;
            return (prev != NULL) ? 1 : 0; /* is colliding node part of its main
                                              position chain? */
        }
        else
    ...
    { /* main position is free, move node */
        *mp = *node;
        gnext(node) = NULL;
...
static int move_number(lua_State *L, Table *t, Node *node)
{
    ...
    if (luai_numeq(cast_num(key), nvalue(key2tval(node))))
    { /* index is int? */
...
static void resize_hashpart(lua_State *L, Table *t, int nhsize)
{
    ...
    if (nhsize > 0)
    { /* round new hashpart size up to next power of two. */
    ...
    { /* hash part might be shrinking */
        ...
        { /* new hashpart size is zero. */
            resizenodevector(L, t, oldhsize, nhsize);
            return;
        }
    }
    /* break old chains, try moving int keys to array part and compact keys into
     * new hashpart */
    for (i = 0; i < oldhsize; i++)
    {
        ...
        gnext(old) = NULL;
        if (ttisnil(gval(old)))
        { /* clear nodes with nil values. */
            setnilvalue(gkey(old));
            continue;
        }
        if (ttisnumber(key2tval(old)))
        { /* try moving the int keys into array part. */
            if (move_number(L, t, old))
                continue;
        }
        if (i >= nhsize)
        {                            /* move all valid keys to indices < nhsize. */
...
int luaH_getn_ro(void *t)
{
    ...
    while (luaR_findentry(t, NULL, i++, NULL)) len++;
    return len;
}
