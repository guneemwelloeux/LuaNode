...
static l_mem propagatemark(global_State* g)
{
    ...
    case LUA_TPROTO:
    {
        ...
        return sizeof(Proto) + sizeof(Proto*) * p->sizep + sizeof(TValue) * p->sizek + sizeof(LocVar) * p->sizelocvars + sizeof(TString*) * p->sizeupvalues + (proto_is_readonly(p) ? 0 : sizeof(Instruction) * p->sizecode +
...
static void markmt(global_State* g)
{
    ...
    for (i = 0; i < NUM_TAGS; i++)
        if (g->mt[i] && !luaR_isrotable(g->mt[i]))
            markobject(g, g->mt[i]);
}
