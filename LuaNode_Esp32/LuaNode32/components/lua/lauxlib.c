...
LUALIB_API void luaL_checkanyfunction(lua_State *L, int narg)
{
    ...
    {
        const char *msg = lua_pushfstring(L, "function or lightfunction expected, got %s", luaL_typename(L, narg));
...
LUALIB_API void luaL_checkanytable(lua_State *L, int narg)
{
    ...
    {
        const char *msg = lua_pushfstring(L, "table or rotable expected, got %s", luaL_typename(L, narg));
        luaL_argerror(L, narg, msg);
    }
}
...
LUALIB_API void(luaL_register_light)(lua_State *L, const char *libname, const luaL_Reg *l)
{
...
LUALIB_API void luaI_openlib(lua_State *L, const char *libname, const luaL_Reg *l, int nup, int ftype)
{
    if (libname)
    {
        int size = libsize(l);
...
LUALIB_API const char *luaL_findtable(lua_State *L, int idx, const char *fname, int szhint)
{
    ...
    do
    {
        ...
        else if (!lua_istable(L, -1) && !lua_isrotable(L, -1))
...
LUALIB_API int luaL_loadfsfile(lua_State *L, const char *filename)
{
    ...
    if (filename)
        vfs_close(lf.f); /* close file (even in case of errors) */
    lua_remove(L, fnameindex);
    return status;
}
