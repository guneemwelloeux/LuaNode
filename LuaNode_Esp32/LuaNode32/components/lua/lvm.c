    ...
    for (loop = 0; loop < MAXTAGLOOP; loop++)
    {
        ...
        { /* `t' is a table? */
            ...
            if (!ttisnil(res) ||                                                                /* result is no nil? */
                (tm = fasttm(L, ttistable(t) ? ((Table *)h)->metatable : (Table *)luaR_getmeta(h), TM_INDEX)) == NULL)
            { /* or no TM? */
                setobj2s(L, val, res);
                return;
            }
    ...
    for (loop = 0; loop < MAXTAGLOOP; loop++)
    {
        ...
        { /* `t' is a table? */
            ...
            if ((oldval && !ttisnil(oldval)) ||                                  /* result is no nil? */
                (tm = fasttm(L, ttistable(t) ? ((Table *)h)->metatable : (Table *)luaR_getmeta(h), TM_NEWINDEX)) ==
                    NULL)
            { /* or no TM? */
                if (oldval)
                {
...
void luaV_concat(lua_State *L, int total, int last)
{
    ...
    if (G(L)->memlimit < max_sizet)
        max_sizet = G(L)->memlimit;
    do
    {
        ...
        switch (GET_OPCODE(i))
        {
            ...
            case OP_LEN:
            {
                ...
                switch (ttype(rb))
                {
                    ...
                    {
                        setnvalue(ra,
                                  ttistable(rb) ? cast_num(luaH_getn(hvalue(rb))) : cast_num(luaH_getn_ro(rvalue(rb))));
                        break;
                    }
                    case LUA_TSTRING:
                    {
