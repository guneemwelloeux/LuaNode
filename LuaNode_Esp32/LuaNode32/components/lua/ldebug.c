...
int luaG_getline(const Proto *f, int pc)
{
    ...
    for (p = f->packedlineinfo; *p && *p != INFO_FILL_BYTE;)
    {
        ...
        { /* line delta */
            ...
            unsigned char sign = *p++ & INFO_SIGN_MASK;
            int shift;
            for (shift = 6; *p & INFO_DELTA_MASK; p++, shift += 7) { delta += (*p & INFO_DELTA_7BITS) << shift; }
            line += sign ? -delta : delta + 2;
        }
        else
        {
...
static int auxgetinfo(lua_State *L, const char *what, lua_Debug *ar, Closure *f, void *plight, CallInfo *ci)
{
    int status = 1;
...
LUA_API int lua_getinfo(lua_State *L, const char *what, lua_Debug *ar)
{
    ...
    {
        if (f != NULL)
            setclvalue(L, L->top, f) else if (plight != NULL) setfvalue(L->top, plight) else setnilvalue(L->top);
        incr_top(L);
    }
    if (strchr(what, 'L'))
...
void luaG_errormsg(lua_State *L)
{
    ...
    { /* is there an error handling function? */
        ...
        if (!ttisfunction(errfunc) && !ttislightfunction(errfunc))
