...
static int stripdebug(lua_State* L, Proto* f, int level)
{
    ...
    case 2:
        ...
        len += f->sizelocvars * (sizeof(struct LocVar) + sizeof(dummy->tsv) + sizeof(struct LocVar*)) + f->sizeupvalues * (sizeof(dummy->tsv) + sizeof(TString*));
        f->sizelocvars = 0;
        f->sizeupvalues = 0;
    }
    return len;
...
LUA_API int luaG_stripdebug(lua_State* L, Proto* f, int level, int recv)
{
    ...
    {
        for (i = 0; i < f->sizep; i++)
            len += luaG_stripdebug(L, f->p[i], level, recv);
    }
...
LUA_API int lua_getinfo(lua_State* L, const char* what, lua_Debug* ar)
{
    ...
    {
        if (f != NULL)
            setclvalue(L, L->top, f) else if (plight != NULL)
                setfvalue(L->top, plight) else setnilvalue(L->top);
        incr_top(L);
    }
    if (strchr(what, 'L'))
