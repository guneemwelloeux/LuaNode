...
void luaS_resize(lua_State *L, int newsize)
{
    ...
    if (luaC_sweepstrgc(L) || newsize == tb->size || is_resizing_strings_gc(L))
        return; /* cannot resize during GC traverse or doesn't need to be
                   resized */
...
static TString *newlstr(lua_State *L, const char *str, size_t l, unsigned int h, int readonly)
{
    TString *ts;
    stringtable *tb;
    ...
    ts = cast(TString *,
              luaM_malloc(L, readonly ? sizeof(char **) + sizeof(TString) : (l + 1) * sizeof(char) + sizeof(TString)));
    ts->tsv.len = l;
    ts->tsv.hash = h;
...
{
    // If the pointer is in a read-only memory and the string is at least 4
    // chars in length, create it as a read-only string instead
