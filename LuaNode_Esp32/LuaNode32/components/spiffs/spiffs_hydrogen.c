...
u32_t SPIFFS_buffer_bytes_for_filedescs(spiffs *fs, u32_t num_descs) { return num_descs * sizeof(spiffs_fd); }
#if SPIFFS_CACHE
...
s32_t SPIFFS_mount(spiffs *fs, spiffs_config *config, u8_t *work, u8_t *fd_space, u32_t fd_space_size, void *cache,
                   u32_t cache_size, spiffs_check_callback check_cb_f)
{
    void *user_data;
    SPIFFS_LOCK(fs);
...
void SPIFFS_clearerr(spiffs *fs) { fs->err_code = SPIFFS_OK; }
...
spiffs_file SPIFFS_open(spiffs *fs, const char *path, spiffs_flags flags, spiffs_mode mode)
{
    ...
    if (res == SPIFFS_OK && (flags & (SPIFFS_O_CREAT | SPIFFS_O_EXCL)) == (SPIFFS_O_CREAT | SPIFFS_O_EXCL))
    {
...
spiffs_file SPIFFS_open_by_page(spiffs *fs, spiffs_page_ix page_ix, spiffs_flags flags, spiffs_mode mode)
{
    ...
    if (res == SPIFFS_ERR_IS_FREE || res == SPIFFS_ERR_DELETED || res == SPIFFS_ERR_NOT_FINALIZED ||
    ...
    if ((fd->flags & SPIFFS_O_DIRECT) == 0)
    {
        if (len < (s32_t)SPIFFS_CFG_LOG_PAGE_SZ(fs))
        {
            ...
            if (fd->cache_page)
            {
                    ...
                    offset + len > fd->cache_page->offset + SPIFFS_CFG_LOG_PAGE_SZ(fs))  // writing beyond cache page
                {
                    ...
                    // new
                    SPIFFS_CACHE_DBG(
                        "CACHE_WR_DUMP: dumping cache page %i for fd %i:%04x, "
                        "boundary viol, offs:%i size:%i\n",
                        fd->cache_page->ix, fd->file_nbr, fd->obj_id, fd->cache_page->offset, fd->cache_page->size);
                    res =
                        spiffs_hydro_write(fs, fd, spiffs_get_cache_page(fs, spiffs_get_cache(fs), fd->cache_page->ix),
            ...
            if (alloc_cpage)
            {
                ...
                if (fd->cache_page)
                {
                    ...
                    SPIFFS_CACHE_DBG(
                        "CACHE_WR_ALLO: allocating cache page %i for fd "
                        "%i:%04x\n",
            ...
            if (fd->cache_page)
            {
                ...
                SPIFFS_CACHE_DBG(
                    "CACHE_WR_WRITE: storing to cache page %i for fd %i:%04x, "
                    "offs %i:%i len %i\n",
                    fd->cache_page->ix, fd->file_nbr, fd->obj_id, offset, offset_in_cpage, len);
        ...
        else
        {
            ...
            if (fd->cache_page)
            {
                ...
                SPIFFS_CACHE_DBG(
                    "CACHE_WR_DUMP: dumping cache page %i for fd %i:%04x, big "
                    "write, offs:%i size:%i\n",
                    fd->cache_page->ix, fd->file_nbr, fd->obj_id, fd->cache_page->offset, fd->cache_page->size);
                res = spiffs_hydro_write(fs, fd, spiffs_get_cache_page(fs, spiffs_get_cache(fs), fd->cache_page->ix),
                                         fd->cache_page->offset, fd->cache_page->size);
                spiffs_cache_fd_release(fs, fd->cache_page);
                SPIFFS_API_CHECK_RES_UNLOCK(fs, res);
                res = spiffs_hydro_write(fs, fd, buf, offset, len);
...
s32_t SPIFFS_lseek(spiffs *fs, spiffs_file fh, s32_t offs, int whence)
{
    ...
    {
        spiffs_page_ix pix;
        res = spiffs_obj_lu_find_id_and_span(fs, fd->obj_id | SPIFFS_OBJ_ID_IX_FLAG, objix_spix, 0, &pix);
        SPIFFS_API_CHECK_RES_UNLOCK(fs, res);
...
static s32_t spiffs_stat_pix(spiffs *fs, spiffs_page_ix pix, spiffs_file fh, spiffs_stat *s)
{
    ...
    spiffs_obj_id obj_id;
    s32_t res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_IX | SPIFFS_OP_C_READ, fh, SPIFFS_PAGE_TO_PADDR(fs, pix),
    ...
    res =
        _spiffs_rd(fs, SPIFFS_OP_T_OBJ_LU | SPIFFS_OP_C_READ, fh, obj_id_addr, sizeof(spiffs_obj_id), (u8_t *)&obj_id);
    SPIFFS_API_CHECK_RES(fs, res);
...
static s32_t spiffs_fflush_cache(spiffs *fs, spiffs_file fh)
{
    ...
    if ((fd->flags & SPIFFS_O_DIRECT) == 0)
    {
        ...
        {
            SPIFFS_CACHE_DBG(
                "CACHE_WR_DUMP: dumping cache page %i for fd %i:%04x, flush, "
                "offs:%i size:%i\n",
                fd->cache_page->ix, fd->file_nbr, fd->obj_id, fd->cache_page->offset, fd->cache_page->size);
            res = spiffs_hydro_write(fs, fd, spiffs_get_cache_page(fs, spiffs_get_cache(fs), fd->cache_page->ix),
                                     fd->cache_page->offset, fd->cache_page->size);
...
s32_t SPIFFS_rename(spiffs *fs, const char *old_path, const char *new_path)
{
    ...
    SPIFFS_API_CHECK_CFG(fs);
    SPIFFS_API_CHECK_MOUNT(fs);
    if (strlen(new_path) > SPIFFS_OBJ_NAME_LEN - 1 || strlen(old_path) > SPIFFS_OBJ_NAME_LEN - 1)
    {
        SPIFFS_API_CHECK_RES(fs, SPIFFS_ERR_NAME_TOO_LONG);
    }
    SPIFFS_LOCK(fs);
    ...
    res =
        spiffs_object_update_index_hdr(fs, fd, fd->obj_id, fd->objix_hdr_pix, 0, (const u8_t *)new_path, 0, &pix_dummy);
...
static s32_t spiffs_read_dir_v(spiffs *fs, spiffs_obj_id obj_id, spiffs_block_ix bix, int ix_entry,
...
struct spiffs_dirent *SPIFFS_readdir(spiffs_DIR *d, struct spiffs_dirent *e)
{
    ...
    res = spiffs_obj_lu_find_entry_visitor(d->fs, d->block, d->entry, SPIFFS_VIS_NO_WRAP, 0, spiffs_read_dir_v, 0, e,
                                           &bix, &entry);
    if (res == SPIFFS_OK)
    {
        d->block = bix;
...
s32_t SPIFFS_info(spiffs *fs, u32_t *total, u32_t *used)
{
    ...
    u32_t total_data_pages =
        (blocks - 2) * (pages_per_block - obj_lu_pages) + 1;  // -2 for spare blocks, +1 for emergency page
...
s32_t SPIFFS_vis(spiffs *fs)
{
    ...
    while (bix < fs->block_count)
    {
        ...
        res = _spiffs_rd(fs, SPIFFS_OP_C_READ | SPIFFS_OP_T_OBJ_LU2, 0, SPIFFS_ERASE_COUNT_PADDR(fs, bix),
