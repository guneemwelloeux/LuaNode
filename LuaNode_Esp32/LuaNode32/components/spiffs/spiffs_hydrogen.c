...
spiffs_file SPIFFS_open_by_page(spiffs* fs, spiffs_page_ix page_ix, spiffs_flags flags, spiffs_mode mode)
{
    ...
    if (res == SPIFFS_ERR_IS_FREE || res == SPIFFS_ERR_DELETED || res == SPIFFS_ERR_NOT_FINALIZED || res == SPIFFS_ERR_NOT_INDEX || res == SPIFFS_ERR_INDEX_SPAN_MISMATCH)
    {
        res = SPIFFS_ERR_NOT_A_FILE;
    }
    if (res < SPIFFS_OK)
...
static s32_t spiffs_stat_pix(spiffs* fs, spiffs_page_ix pix, spiffs_file fh, spiffs_stat* s)
{
    ...
    u32_t obj_id_addr = SPIFFS_BLOCK_TO_PADDR(fs, SPIFFS_BLOCK_FOR_PAGE(fs, pix)) + SPIFFS_OBJ_LOOKUP_ENTRY_FOR_PAGE(fs, pix) * sizeof(spiffs_obj_id);
    ...
    void* user_var_p)
{
    ...
    if (res != SPIFFS_OK)
        return res;
    if ((obj_id & SPIFFS_OBJ_ID_IX_FLAG) && objix_hdr.p_hdr.span_ix == 0 && (objix_hdr.p_hdr.flags & (SPIFFS_PH_FLAG_DELET | SPIFFS_PH_FLAG_FINAL | SPIFFS_PH_FLAG_IXDELE)) == (SPIFFS_PH_FLAG_DELET | SPIFFS_PH_FLAG_IXDELE))
    {
