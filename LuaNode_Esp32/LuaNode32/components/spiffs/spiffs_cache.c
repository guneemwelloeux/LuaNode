...
static spiffs_cache_page *spiffs_cache_page_get(spiffs *fs, spiffs_page_ix pix)
{
    ...
    for (i = 0; i < cache->cpage_count; i++)
    {
        ...
        if ((cache->cpage_use_map & (1 << i)) && (cp->flags & SPIFFS_CACHE_FLAG_TYPE_WR) == 0 && cp->pix == pix)
        {
...
static s32_t spiffs_cache_page_free(spiffs *fs, int ix, u8_t write_back)
{
    ...
    {
        if (write_back && (cp->flags & SPIFFS_CACHE_FLAG_TYPE_WR) == 0 && (cp->flags & SPIFFS_CACHE_FLAG_DIRTY))
        {
...
static s32_t spiffs_cache_page_remove_oldest(spiffs *fs, u8_t flag_mask, u8_t flags)
{
    ...
    {
        spiffs_cache_page *cp = spiffs_get_cache_page_hdr(fs, cache, i);
        if ((cache->last_access - cp->last_access) > oldest_val && (cp->flags & flag_mask) == flags)
        {
...
// allocates a new cached page and returns it, or null if all cache pages are
// busy
...
s32_t spiffs_phys_rd(spiffs *fs, u8_t op, spiffs_file fh, u32_t addr, u32_t len, u8_t *dst)
{
    ...
    else
    {
        ...
        s32_t res2 = SPIFFS_HAL_READ(fs, addr - SPIFFS_PADDR_TO_PAGE_OFFSET(fs, addr), SPIFFS_CFG_LOG_PAGE_SZ(fs),
...
s32_t spiffs_phys_wr(spiffs *fs, u8_t op, spiffs_file fh, u32_t addr, u32_t len, u8_t *src)
{
    ...
    if (cp && (op & SPIFFS_OP_COM_MASK) != SPIFFS_OP_C_WRTHRU)
    {
        ...
        if ((op & SPIFFS_OP_COM_MASK) == SPIFFS_OP_C_DELE && (op & SPIFFS_OP_TYPE_MASK) != SPIFFS_OP_T_OBJ_LU)
        {
...
spiffs_cache_page *spiffs_cache_page_get_by_fd(spiffs *fs, spiffs_fd *fd)
{
    ...
    {
        spiffs_cache_page *cp = spiffs_get_cache_page_hdr(fs, cache, i);
        if ((cache->cpage_use_map & (1 << i)) && (cp->flags & SPIFFS_CACHE_FLAG_TYPE_WR) && cp->obj_id == fd->obj_id)
        {
            return cp;
        }
    }
...
void spiffs_cache_init(spiffs *fs)
{
    ...
    for (i = 0; i < cache.cpage_count; i++) { spiffs_get_cache_page_hdr(fs, c, i)->ix = i; }
}
