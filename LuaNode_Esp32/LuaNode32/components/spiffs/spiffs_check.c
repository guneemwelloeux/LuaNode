 ...
 * headers). If an object index is found but not its header, the object index is
 * deleted. This is critical for the following page consistency check. Page
 * consistency Checks for pages that ought to be indexed, ought not to be
 * indexed, are multiple indexed
 *
 *
...
#define CHECK_CB(_fs, _type, _rep, _arg1, _arg2)                         \
    ...
    {                                                                    \
        if ((_fs)->check_cb_f)                                           \
            (_fs)->check_cb_f((_fs), (_type), (_rep), (_arg1), (_arg2)); \
    } while (0)
#else
#define CHECK_CB(_fs, _type, _rep, _arg1, _arg2)                  \
    do                                                            \
    {                                                             \
        if ((_fs)->check_cb_f)                                    \
            (_fs)->check_cb_f((_type), (_rep), (_arg1), (_arg2)); \
    } while (0)
#endif
...
// destroys fs->lu_work
static s32_t spiffs_object_get_data_page_index_reference(spiffs *fs, spiffs_obj_id obj_id, spiffs_span_ix data_spix,
                                                         spiffs_page_ix *pix, spiffs_page_ix *objix_pix)
{
    s32_t res;
...
static s32_t spiffs_rewrite_page(spiffs *fs, spiffs_page_ix cur_pix, spiffs_page_header *p_hdr, spiffs_page_ix *new_pix)
{
    ...
    res = spiffs_phys_cpy(fs, 0, SPIFFS_PAGE_TO_PADDR(fs, *new_pix) + sizeof(spiffs_page_header),
                          SPIFFS_PAGE_TO_PADDR(fs, cur_pix) + sizeof(spiffs_page_header), SPIFFS_DATA_PAGE_SIZE(fs));
    SPIFFS_CHECK_RES(res);
    return res;
}
                                  ...
                                  spiffs_page_ix new_data_pix, spiffs_page_ix objix_pix)
{
    ...
    // load index
    res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_LU2 | SPIFFS_OP_C_READ, 0, SPIFFS_PAGE_TO_PADDR(fs, objix_pix),
    ...
    {
        ((spiffs_page_ix *)((u8_t *)fs->lu_work + sizeof(spiffs_page_object_ix)))[SPIFFS_OBJ_IX_ENTRY(fs, data_spix)] =
            new_data_pix;
    }
...
static s32_t spiffs_delete_obj_lazy(spiffs *fs, spiffs_obj_id obj_id)
{
    ...
    res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_LU | SPIFFS_OP_C_UPDT, 0,
                     SPIFFS_PAGE_TO_PADDR(fs, objix_hdr_pix) + offsetof(spiffs_page_header, flags), sizeof(u8_t),
                     (u8_t *)&flags);
    return res;
}
...
static s32_t spiffs_lookup_check_validate(spiffs *fs, spiffs_obj_id lu_obj_id, spiffs_page_header *p_hdr,
                                          spiffs_page_ix cur_pix, spiffs_block_ix cur_block, int cur_entry,
                                          int *reload_lu)
{
    (void)cur_block;
    (void)cur_entry;
        ...
        ((lu_obj_id == SPIFFS_OBJ_ID_FREE) && (p_hdr->flags & SPIFFS_PH_FLAG_USED) == 0))
    {
        ...
        *reload_lu = 1;
        ...
        if (p_hdr->flags & SPIFFS_PH_FLAG_INDEX)
        {
            ...
            else
            {
                ...
                if (ref_pix == cur_pix)
                {
                    ...
                    *reload_lu = 1;
                    SPIFFS_CHECK_DBG(
                        "LU: FIXUP: %04x rewritten to %04x, affected objix_pix "
                        "%04x\n",
        ...
        else
        {
            ...
            // index page can be removed if other index with same obj_id and
            // spanix is found
            ...
            if (res == SPIFFS_ERR_NOT_FOUND)
            {
                ...
                {  // ignore other errors
                    ...
                    res = spiffs_rewrite_page(fs, cur_pix, p_hdr, &new_pix);
                    SPIFFS_CHECK_DBG(
                        "LU: FIXUP: ix page with data not found elsewhere, "
    ...
    if (lu_obj_id != SPIFFS_OBJ_ID_FREE && lu_obj_id != SPIFFS_OBJ_ID_DELETED)
    {
        ...
        if ((p_hdr->obj_id | SPIFFS_OBJ_ID_IX_FLAG) != (lu_obj_id | SPIFFS_OBJ_ID_IX_FLAG))
        {
            ...
            delete_page = 1;
            if ((p_hdr->flags & SPIFFS_PH_FLAG_DELET) == 0 || (p_hdr->flags & SPIFFS_PH_FLAG_FINAL) ||
            ...
            else
            {
                if (p_hdr->flags & SPIFFS_PH_FLAG_INDEX)
                {
                    ...
                    {
                        SPIFFS_CHECK_RES(res);
                        //   if found, rewrite page with object id, update
                        //   index, and delete current
                        if (ref_pix == cur_pix)
                        {
                            spiffs_page_ix new_pix;
                            res = spiffs_rewrite_page(fs, cur_pix, p_hdr, &new_pix);
                ...
                {
                    // else if index, check for other pages with both obj_id's
                    // and spanix
                    ...
                    if (objix_pix_ph == 0 || objix_pix_lu == 0)
                    {
                        ...
                        // span index
                        res =
                            spiffs_obj_lu_find_id_and_span(fs, lu_obj_id & ~SPIFFS_OBJ_ID_IX_FLAG, 0, 0, &data_pix_lu);
                        if (res == SPIFFS_ERR_NOT_FOUND)
                        {
                            res = SPIFFS_OK;
                            objix_pix_lu = 0;
                            ...
                            (objix_pix_lu == 0 && data_pix_ph && objix_pix_ph == 0))
                        {
                            ...
                            SPIFFS_CHECK_DBG(
                                "LU: FIXUP: rewrite page %04x as %04x to pix "
                                "%04x\n",
                 ...
                 ((lu_obj_id & SPIFFS_OBJ_ID_IX_FLAG) == 0 && (p_hdr->flags & SPIFFS_PH_FLAG_INDEX) == 0))
        {
            ...
            {
                SPIFFS_CHECK_DBG(
                    "LU: FIXUP: other index and data page exists, simply "
                    "remove\n");
            }
            else
        ...
        else if ((p_hdr->flags & SPIFFS_PH_FLAG_FINAL))
        {
            ...
            *reload_lu = 1;
            ...
            else
            {
                ...
                {
                    SPIFFS_CHECK_DBG(
                        "LU: FIXUP: other finalized page is referred, just "
                        "delete\n");
                    delete_page = 1;
                }
                else
                {
                    ...
                    // just finalize
                    SPIFFS_CHECK_DBG(
                        "LU: FIXUP: unfinalized page is referred, "
                        "finalizing\n");
                    CHECK_CB(fs, SPIFFS_CHECK_LOOKUP, SPIFFS_CHECK_FIX_LOOKUP, p_hdr->obj_id, p_hdr->span_ix);
                    u8_t flags = 0xff & ~SPIFFS_PH_FLAG_FINAL;
                    res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_DA | SPIFFS_OP_C_UPDT, 0,
                                     SPIFFS_PAGE_TO_PADDR(fs, cur_pix) + offsetof(spiffs_page_header, flags),
                                   ...
                                   const void *user_const_p, void *user_var_p)
{
    ...
    CHECK_CB(fs, SPIFFS_CHECK_LOOKUP, SPIFFS_CHECK_PROGRESS, (cur_block * 256) / fs->block_count, 0);

    // load header
    res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_LU2 | SPIFFS_OP_C_READ, 0, SPIFFS_PAGE_TO_PADDR(fs, cur_pix),
...
// checked for validity. If an object index header page is found, this is also
// checked
...
// Scans all pages (except lu pages), reserves 4 bits in working memory for each
// page bit 0: 0 == FREE|DELETED, 1 == USED bit 1: 0 == UNREFERENCED, 1 ==
// REFERENCED bit 2: 0 == NOT_INDEX,    1 == INDEX bit 3: unused A consistent
// file system will have only pages being
...
static s32_t spiffs_page_consistency_check_i(spiffs *fs)
{
    ...
    while (pix_offset < SPIFFS_PAGES_PER_BLOCK(fs) * fs->block_count)
    {
        ...
        while (!restart && cur_block < fs->block_count)
        {
            ...
            {
                // if ((cur_pix & 0xff) == 0)
                //  SPIFFS_CHECK_DBG("PA: processing pix %08x, block %08x of pix
                //  %08x, block %08x\n",
                //      cur_pix, cur_block, SPIFFS_PAGES_PER_BLOCK(fs) *
                //      fs->block_count, fs->block_count);
                ...
                if (within_range && (p_hdr.flags & SPIFFS_PH_FLAG_DELET) && (p_hdr.flags & SPIFFS_PH_FLAG_USED) == 0)
                {
                    // used
                    fs->work[pix_byte_ix] |= (1 << (pix_bit_ix + 0));
                }
                if ((p_hdr.flags & SPIFFS_PH_FLAG_DELET) && (p_hdr.flags & SPIFFS_PH_FLAG_IXDELE) &&
                    (p_hdr.flags & (SPIFFS_PH_FLAG_INDEX | SPIFFS_PH_FLAG_USED)) == 0)
                {
                    ...
                    if (p_hdr.span_ix == 0)
                    {
                        ...
                        data_spix_offset = 0;
                        object_page_index =
                            (spiffs_page_ix *)((u8_t *)fs->lu_work + sizeof(spiffs_page_object_ix_header));
                    }
                    else
                    {
                        // object page index
                    ...
                    for (i = 0; !restart && i < entries; i++)
                    {
                            ...
                            (rpix_within_range && SPIFFS_IS_LOOKUP_PAGE(fs, rpix)))
                        {
                            // bad reference
                            SPIFFS_CHECK_DBG(
                                "PA: pix %04x bad pix / LU referenced from "
                                "page %04x\n",
                                rpix, cur_pix);
                            ...
                            if (data_pix == 0)
                            {
                                ...
                                SPIFFS_CHECK_RES(res);
                                SPIFFS_CHECK_DBG(
                                    "PA: FIXUP: found no existing data page, "
                                    "created new @ %04x\n",
                                    data_pix);
                            }
                            // remap index
                            ...
                            {
                                CHECK_CB(fs, SPIFFS_CHECK_PAGE, SPIFFS_CHECK_FIX_INDEX, objix_p_hdr->obj_id,
                                         objix_p_hdr->span_ix);
                            }
                            SPIFFS_CHECK_RES(res);
                            restart = 1;
                        ...
                        else if (rpix_within_range)
                        {
                            ...
                            res =
                                _spiffs_rd(fs, SPIFFS_OP_T_OBJ_LU2 | SPIFFS_OP_C_READ, 0,
                                           SPIFFS_PAGE_TO_PADDR(fs, rpix), sizeof(spiffs_page_header), (u8_t *)&rp_hdr);
                            SPIFFS_CHECK_RES(res);
                            ...
                            {
                                SPIFFS_CHECK_DBG(
                                    "PA: pix %04x has inconsistent page header "
                                    "ix id/span:%04x/%04x, ref "
                                    "id/span:%04x/%04x flags:%02x\n",
                                    rpix, p_hdr.obj_id & ~SPIFFS_OBJ_ID_IX_FLAG, data_spix_offset + i, rp_hdr.obj_id,
                                    rp_hdr.span_ix, rp_hdr.flags);
                                ...
                                if (data_pix == 0)
                                {
                                    ...
                                    SPIFFS_CHECK_DBG(
                                        "PA: FIXUP: index bad, delete object "
                                        "id %04x\n",
                                        p_hdr.obj_id);
                                ...
                                else
                                {
                                    ...
                                    SPIFFS_CHECK_DBG(
                                        ...
                                        "%04x, rewrite ix pix %04x id %04x\n",
                                        data_pix, cur_pix, p_hdr.obj_id);
                                    res =
                                        spiffs_rewrite_index(fs, p_hdr.obj_id, data_spix_offset + i, data_pix, cur_pix);
                                    if (res <= _SPIFFS_ERR_CHECK_FIRST && res > _SPIFFS_ERR_CHECK_LAST)
                                    {
                                        // index bad also, cannot mend this file
                                        SPIFFS_CHECK_DBG(
                                            "PA: FIXUP: index bad %i, cannot "
                                            "mend!\n",
                                            res);
                                        CHECK_CB(fs, SPIFFS_CHECK_PAGE, SPIFFS_CHECK_DELETE_BAD_FILE, p_hdr.obj_id, 0);
                                        res = spiffs_delete_obj_lazy(fs, p_hdr.obj_id);
                                    ...
                                    {
                                        CHECK_CB(fs, SPIFFS_CHECK_PAGE, SPIFFS_CHECK_FIX_INDEX, p_hdr.obj_id,
                                                 p_hdr.span_ix);
                                    }
                                    SPIFFS_CHECK_RES(res);
                                    restart = 1;
                            ...
                            else
                            {
                                ...
                                {
                                    SPIFFS_CHECK_DBG(
                                        "PA: pix %04x multiple referenced from "
                                        "page %04x\n",
                                        rpix, cur_pix);
                                    // Here, we should have fixed all broken
                                    // references - getting this means there
                                    // must be multiple files with same object
                                    // id. Only solution is to delete the object
                                    // which is referring to this page
                                    SPIFFS_CHECK_DBG(
                                        "PA: FIXUP: removing object %04x and "
                                        "page %04x\n",
                ...
                for (bit_ix = 0; !restart && bit_ix < 8 / bits; bit_ix++)
                {
                    ...
                    if (bitmask == 0x1)
                    {
                        ...
                        res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_LU2 | SPIFFS_OP_C_READ, 0,
                                         SPIFFS_PAGE_TO_PADDR(fs, cur_pix), sizeof(spiffs_page_header), (u8_t *)&p_hdr);
                        SPIFFS_CHECK_RES(res);

                        res = spiffs_object_get_data_page_index_reference(fs, p_hdr.obj_id, p_hdr.span_ix, &rpix,
                                                                          &objix_pix);
                        if (res == SPIFFS_OK)
                        {
                            if (((rpix == (spiffs_page_ix)-1 || rpix > SPIFFS_MAX_PAGES(fs)) ||
                                 (SPIFFS_IS_LOOKUP_PAGE(fs, rpix))))
                            {
                                ...
                                SPIFFS_CHECK_DBG(
                                    "PA: corresponding ref is bad: %04x, "
                                    "rewrite to this %04x\n",
                                    rpix, cur_pix);
                            }
                            else
                            {
                                ...
                                spiffs_page_header rp_hdr;
                                res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_LU2 | SPIFFS_OP_C_READ, 0,
                                                 SPIFFS_PAGE_TO_PADDR(fs, rpix), sizeof(spiffs_page_header),
                                                 (u8_t *)&rp_hdr);
                                SPIFFS_CHECK_RES(res);
                                if (((p_hdr.obj_id & ~SPIFFS_OBJ_ID_IX_FLAG) == rp_hdr.obj_id) &&
                                    ((rp_hdr.flags & (SPIFFS_PH_FLAG_INDEX | SPIFFS_PH_FLAG_DELET |
                                                      SPIFFS_PH_FLAG_USED | SPIFFS_PH_FLAG_FINAL)) ==
                                     (SPIFFS_PH_FLAG_INDEX | SPIFFS_PH_FLAG_DELET)))
                                {
                                    // pointing to something else valid, just
                                    // delete this page then
                                    SPIFFS_CHECK_DBG(
                                        "PA: corresponding ref is good but "
                                ...
                                else
                                {
                                    ...
                                    if (rpix != cur_pix)
                                    {
                                        SPIFFS_CHECK_DBG(
                                            ...
                                            "%04x\n",
                                            rpix, (rp_hdr.flags & SPIFFS_PH_FLAG_INDEX) ? "" : "INDEX ",
                                            (rp_hdr.flags & SPIFFS_PH_FLAG_DELET) ? "" : "DELETED ",
                                            (rp_hdr.flags & SPIFFS_PH_FLAG_USED) ? "NOTUSED " : "",
                                            (rp_hdr.flags & SPIFFS_PH_FLAG_FINAL) ? "NOTFINAL " : "", cur_pix);
                                        rewrite_ix_to_this = 1;
                                    }
                                    else
                                    {
                        ...
                        if (rewrite_ix_to_this)
                        {
                            ...
                            // this page
                            SPIFFS_CHECK_DBG(
                                "PA: FIXUP: rewrite index id %04x data spix "
                                "%04x to point to this pix: %04x\n",
                                                     ...
                                                     int cur_entry, const void *user_const_p, void *user_var_p)
{
    ...
    CHECK_CB(fs, SPIFFS_CHECK_INDEX, SPIFFS_CHECK_PROGRESS, (cur_block * 256) / fs->block_count, 0);
    ...
    if (obj_id != SPIFFS_OBJ_ID_FREE && obj_id != SPIFFS_OBJ_ID_DELETED && (obj_id & SPIFFS_OBJ_ID_IX_FLAG))
    {
        ...
        if (p_hdr.span_ix == 0 && (p_hdr.flags & (SPIFFS_PH_FLAG_INDEX | SPIFFS_PH_FLAG_FINAL | SPIFFS_PH_FLAG_DELET |
                                                  SPIFFS_PH_FLAG_IXDELE)) == (SPIFFS_PH_FLAG_DELET))
        {
            SPIFFS_CHECK_DBG(
        ...
        if ((p_hdr.flags & (SPIFFS_PH_FLAG_INDEX | SPIFFS_PH_FLAG_FINAL | SPIFFS_PH_FLAG_DELET |
                            SPIFFS_PH_FLAG_IXDELE)) == (SPIFFS_PH_FLAG_DELET | SPIFFS_PH_FLAG_IXDELE))
        {
            return res_c;
        }
        ...
        {  // span index
            ...
            {
                SPIFFS_CHECK_DBG(
                    "IX: FIXUP: pix %04x, obj id:%04x spix:%04x is orphan "
                    "index - deleting\n",
                    cur_pix, obj_id, p_hdr.span_ix);
...
// header is searched for. If no such page exists, the index page cannot be
// reached as no index header exists and must be deleted.
s32_t spiffs_object_index_consistency_check(spiffs *fs)
{
    s32_t res = SPIFFS_OK;
    // impl note:
    // fs->work is used for a temporary object index memory, listing found
    // object ids and indicating whether they can be reached or not. Acting as a
    // fifo if object ids cannot fit. In the temporary object index memory,
