...
static s32_t spiffs_page_data_check(spiffs *fs, spiffs_fd *fd, spiffs_page_ix pix, spiffs_span_ix spix)
{
    ...
    res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_DA | SPIFFS_OP_C_READ, fd->file_nbr, SPIFFS_PAGE_TO_PADDR(fs, pix),
                     sizeof(spiffs_page_header), (u8_t *)&ph);
...
#if SPIFFS_PAGE_CHECK
    spiffs_page_header ph;
    res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_IX | SPIFFS_OP_C_READ, fd->file_nbr, SPIFFS_PAGE_TO_PADDR(fs, pix),
                     sizeof(spiffs_page_header), (u8_t *)&ph);
    SPIFFS_CHECK_RES(res);
...
s32_t spiffs_phys_rd(spiffs *fs, u32_t addr, u32_t len, u8_t *dst) { return SPIFFS_HAL_READ(fs, addr, len, dst); }

s32_t spiffs_phys_wr(spiffs *fs, u32_t addr, u32_t len, u8_t *src) { return SPIFFS_HAL_WRITE(fs, addr, len, src); }
...
// Iterate over object lookup pages in each block until a given object id entry
// is found. When found, the visitor function is called with block index, entry
// index and user data. If visitor returns SPIFFS_VIS_CONTINUE, the search goes
// on. Otherwise, the search will be ended and visitor's return code is returned
// to caller. If no visitor is given (0) the search returns on first entry with
// matching object id. If no match is found in all look up, SPIFFS_VIS_END is
// returned.
...
// @param flags                 ored combination of SPIFFS_VIS_CHECK_ID,
// SPIFFS_VIS_CHECK_PH,
//                              SPIFFS_VIS_NO_WRAP
// @param obj_id                argument object id
// @param v                     visitor callback function
// @param user_const_p          any const pointer, passed to the callback
// visitor function
// @param user_var_p            any pointer, passed to the callback visitor
// function
// @param block_ix              reported block index where match was found
// @param lu_entry              reported look up index where match was found
s32_t spiffs_obj_lu_find_entry_visitor(spiffs *fs, spiffs_block_ix starting_block, int starting_lu_entry, u8_t flags,
                                       spiffs_obj_id obj_id, spiffs_visitor_f v, const void *user_const_p,
                                       void *user_var_p, spiffs_block_ix *block_ix, int *lu_entry)
{
    s32_t res = SPIFFS_OK;
    ...
    while (res == SPIFFS_OK && entry_count > 0)
    {
        ...
        while (res == SPIFFS_OK && obj_lookup_page < (int)SPIFFS_OBJ_LOOKUP_PAGES(fs))
        {
            ...
            res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_LU | SPIFFS_OP_C_READ, 0,
                             cur_block_addr + SPIFFS_PAGE_TO_PADDR(fs, obj_lookup_page), SPIFFS_CFG_LOG_PAGE_SZ(fs),
                             fs->lu_work);
            // check each entry
            while (res == SPIFFS_OK && cur_entry - entry_offset < entries_per_page &&  // for non-last obj lookup pages
                   cur_entry < (int)SPIFFS_OBJ_LOOKUP_MAX_ENTRIES(fs))                 // for last obj lookup page
            {
                if ((flags & SPIFFS_VIS_CHECK_ID) == 0 || obj_lu_buf[cur_entry - entry_offset] == obj_id)
                {
                    ...
                    if (v)
                    {
                        res = v(fs, (flags & SPIFFS_VIS_CHECK_PH) ? obj_id : obj_lu_buf[cur_entry - entry_offset],
                                cur_block, cur_entry, user_const_p, user_var_p);
...
s32_t spiffs_erase_block(spiffs *fs, spiffs_block_ix bix)
{
    ...
    res = _spiffs_wr(fs, SPIFFS_OP_C_WRTHRU | SPIFFS_OP_T_OBJ_LU2, 0, SPIFFS_ERASE_COUNT_PADDR(fs, bix),
    ...
    res = _spiffs_wr(fs, SPIFFS_OP_C_WRTHRU | SPIFFS_OP_T_OBJ_LU2, 0, SPIFFS_MAGIC_PADDR(fs, bix),
...
s32_t spiffs_probe(spiffs_config *cfg)
{
    s32_t res;
    u32_t paddr;
...
static s32_t spiffs_obj_lu_scan_v(spiffs *fs, spiffs_obj_id obj_id, spiffs_block_ix bix, int ix_entry,
...
s32_t spiffs_obj_lu_scan(spiffs *fs)
{
    ...
    while (bix < fs->block_count)
    {
#if SPIFFS_USE_MAGIC
        spiffs_obj_id magic;
        res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_LU2 | SPIFFS_OP_C_READ, 0, SPIFFS_MAGIC_PADDR(fs, bix),
                         sizeof(spiffs_obj_id), (u8_t *)&magic);
        ...
        if (magic != SPIFFS_MAGIC(fs, bix))
        {
            ...
            {
                // allow one unerased block as it might be powered down during
                // an erase
                unerased_bix = bix;
            }
            else
        ...
        res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_LU2 | SPIFFS_OP_C_READ, 0, SPIFFS_ERASE_COUNT_PADDR(fs, bix),
    ...
    res = spiffs_obj_lu_find_entry_visitor(fs, 0, 0, 0, 0, spiffs_obj_lu_scan_v, 0, 0, &bix, &entry);
...
// Iterate over object lookup pages in each block until a free object id entry
// is found
s32_t spiffs_obj_lu_find_free(spiffs *fs, spiffs_block_ix starting_block, int starting_lu_entry,
                              spiffs_block_ix *block_ix, int *lu_entry)
{
    ...
    res = spiffs_obj_lu_find_id(fs, starting_block, starting_lu_entry, SPIFFS_OBJ_ID_FREE, block_ix, lu_entry);
    if (res == SPIFFS_OK)
    {
...
// Iterate over object lookup pages in each block until a given object id entry
// is found
s32_t spiffs_obj_lu_find_id(spiffs *fs, spiffs_block_ix starting_block, int starting_lu_entry, spiffs_obj_id obj_id,
                            spiffs_block_ix *block_ix, int *lu_entry)
{
    s32_t res = spiffs_obj_lu_find_entry_visitor(fs, starting_block, starting_lu_entry, SPIFFS_VIS_CHECK_ID, obj_id, 0,
                                                 0, 0, block_ix, lu_entry);
    if (res == SPIFFS_VIS_END)
    {
...
static s32_t spiffs_obj_lu_find_id_and_span_v(spiffs *fs, spiffs_obj_id obj_id, spiffs_block_ix bix, int ix_entry,
                                              const void *user_const_p, void *user_var_p)
{
    s32_t res;
    spiffs_page_header ph;
    spiffs_page_ix pix = SPIFFS_OBJ_LOOKUP_ENTRY_TO_PIX(fs, bix, ix_entry);
    res = _spiffs_rd(fs, 0, SPIFFS_OP_T_OBJ_LU2 | SPIFFS_OP_C_READ, SPIFFS_PAGE_TO_PADDR(fs, pix),
                     sizeof(spiffs_page_header), (u8_t *)&ph);
    SPIFFS_CHECK_RES(res);
    if (ph.obj_id == obj_id && ph.span_ix == *((spiffs_span_ix *)user_var_p) &&
...
// Iterate over object lookup pages in each block until a given object id entry
// is found
s32_t spiffs_obj_lu_find_id_and_span(spiffs *fs, spiffs_obj_id obj_id, spiffs_span_ix spix,
                                     spiffs_page_ix exclusion_pix, spiffs_page_ix *pix)
{
    s32_t res;
    spiffs_block_ix bix;
    int entry;

    res = spiffs_obj_lu_find_entry_visitor(fs, fs->cursor_block_ix, fs->cursor_obj_lu_entry, SPIFFS_VIS_CHECK_ID,
                                           obj_id, spiffs_obj_lu_find_id_and_span_v, exclusion_pix ? &exclusion_pix : 0,
                                           &spix, &bix, &entry);
...
// Find object lookup entry containing given id and span index in page headers
// only Iterate over object lookup pages in each block until a given object id
// entry is found
s32_t spiffs_obj_lu_find_id_and_span_by_phdr(spiffs *fs, spiffs_obj_id obj_id, spiffs_span_ix spix,
                                             spiffs_page_ix exclusion_pix, spiffs_page_ix *pix)
{
    s32_t res;
    spiffs_block_ix bix;
    ...
    res = spiffs_obj_lu_find_entry_visitor(fs, fs->cursor_block_ix, fs->cursor_obj_lu_entry, SPIFFS_VIS_CHECK_PH,
                                           obj_id, spiffs_obj_lu_find_id_and_span_v, exclusion_pix ? &exclusion_pix : 0,
                                           &spix, &bix, &entry);
...
// data may be NULL; where only page header is stored, len and page_offs is
// ignored
s32_t spiffs_page_allocate_data(spiffs *fs, spiffs_obj_id obj_id, spiffs_page_header *ph, u8_t *data, u32_t len,
                                u32_t page_offs, u8_t finalize, spiffs_page_ix *pix)
{
    s32_t res = SPIFFS_OK;
    spiffs_block_ix bix;
    int entry;
    ...
    res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_LU | SPIFFS_OP_C_UPDT, 0,
                     SPIFFS_BLOCK_TO_PADDR(fs, bix) + entry * sizeof(spiffs_obj_id), sizeof(spiffs_obj_id),
                     (u8_t *)&obj_id);
    SPIFFS_CHECK_RES(res);
    ...
    res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_DA | SPIFFS_OP_C_UPDT, 0, SPIFFS_OBJ_LOOKUP_ENTRY_TO_PADDR(fs, bix, entry),
                     sizeof(spiffs_page_header), (u8_t *)ph);
    SPIFFS_CHECK_RES(res);
    ...
    if (data)
    {
        res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_DA | SPIFFS_OP_C_UPDT, 0,
                         SPIFFS_OBJ_LOOKUP_ENTRY_TO_PADDR(fs, bix, entry) + sizeof(spiffs_page_header) + page_offs, len,
                         data);
        SPIFFS_CHECK_RES(res);
    }
...
// Page data is given in param page. If page data is null, provided header is
// used for metainfo and page data is physically copied.
s32_t spiffs_page_move(spiffs *fs, spiffs_file fh, u8_t *page_data, spiffs_obj_id obj_id, spiffs_page_header *page_hdr,
                       spiffs_page_ix src_pix, spiffs_page_ix *dst_pix)
{
    ...
    if (page_data)
    {
        ...
        res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_DA | SPIFFS_OP_C_UPDT, 0, SPIFFS_PAGE_TO_PADDR(fs, free_pix),
    ...
    if (was_final)
    {
        ...
        res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_DA | SPIFFS_OP_C_UPDT, fh,
                         SPIFFS_PAGE_TO_PADDR(fs, free_pix) + offsetof(spiffs_page_header, flags), sizeof(u8_t),
                         (u8_t *)&p_hdr->flags);
        SPIFFS_CHECK_RES(res);
    }
    // mark source deleted
...
s32_t spiffs_page_delete(spiffs *fs, spiffs_page_ix pix)
{
    ...
    res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_LU | SPIFFS_OP_C_DELE, 0,
                     ...
                     sizeof(spiffs_obj_id), (u8_t *)&d_obj_id);
    SPIFFS_CHECK_RES(res);
    ...
    res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_DA | SPIFFS_OP_C_DELE, 0,
                     SPIFFS_PAGE_TO_PADDR(fs, pix) + offsetof(spiffs_page_header, flags), sizeof(u8_t),
                     (u8_t *)&hdr.flags);
...
s32_t spiffs_object_create(spiffs *fs, spiffs_obj_id obj_id, const u8_t name[SPIFFS_OBJ_NAME_LEN], spiffs_obj_type type,
                           spiffs_page_ix *objix_hdr_pix)
{
    ...
    // update page
    res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_DA | SPIFFS_OP_C_UPDT, 0, SPIFFS_OBJ_LOOKUP_ENTRY_TO_PADDR(fs, bix, entry),
                     sizeof(spiffs_page_object_ix_header), (u8_t *)&oix_hdr);
...
s32_t spiffs_object_update_index_hdr(spiffs *fs, spiffs_fd *fd, spiffs_obj_id obj_id, spiffs_page_ix objix_hdr_pix,
                                     u8_t *new_objix_hdr_data, const u8_t name[SPIFFS_OBJ_NAME_LEN], u32_t size,
                                     spiffs_page_ix *new_pix)
{
    ...
    else
    {
        ...
        res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_IX | SPIFFS_OP_C_READ, fd->file_nbr,
                         SPIFFS_PAGE_TO_PADDR(fs, objix_hdr_pix), SPIFFS_CFG_LOG_PAGE_SZ(fs), fs->work);
        SPIFFS_CHECK_RES(res);
    ...
    if (res == SPIFFS_OK)
    {
        ...
        spiffs_cb_object_event(fs, fd, SPIFFS_EV_IX_UPD, obj_id, objix_hdr->p_hdr.span_ix, new_objix_hdr_pix,
                               objix_hdr->size);
        if (fd)
            fd->objix_hdr_pix = new_objix_hdr_pix;  // if this is not in the registered cluster
    }
...
void spiffs_cb_object_event(spiffs *fs, spiffs_fd *fd, int ev, spiffs_obj_id obj_id_raw, spiffs_span_ix spix,
                            spiffs_page_ix new_pix, u32_t new_size)
{
    (void)fd;
    ...
    for (i = 0; i < fs->fd_count; i++)
    {
        ...
        if (cur_fd->file_nbr == 0 || (cur_fd->obj_id & ~SPIFFS_OBJ_ID_IX_FLAG) != obj_id)
            continue;
        if (spix == 0)
        {
            ...
            {
                SPIFFS_DBG(
                    "       callback: setting fd %i:%04x objix_hdr_pix to "
                    "%04x, size:%i\n",
        ...
        if (cur_fd->cursor_objix_spix == spix)
        {
            ...
            {
                SPIFFS_DBG(
                    "       callback: setting fd %i:%04x span:%04x objix_pix "
                    "to %04x\n",
...
// Open object by id
s32_t spiffs_object_open_by_id(spiffs *fs, spiffs_obj_id obj_id, spiffs_fd *fd, spiffs_flags flags, spiffs_mode mode)
{
    s32_t res = SPIFFS_OK;
    spiffs_page_ix pix;
...
s32_t spiffs_object_open_by_page(spiffs *fs, spiffs_page_ix pix, spiffs_fd *fd, spiffs_flags flags, spiffs_mode mode)
{
    (void)mode;
    s32_t res = SPIFFS_OK;
    spiffs_page_object_ix_header oix_hdr;
...
s32_t spiffs_object_append(spiffs_fd *fd, u32_t offset, u8_t *data, u32_t len)
{
    ...
    while (res == SPIFFS_OK && written < len)
    {
        ...
        if (cur_objix_spix != prev_objix_spix)
        {
            ...
            // within this clause we return directly if something fails, object
            // index mess-up
            if (written > 0)
            {
                ...
                if (prev_objix_spix == 0)
                {
                    ...
                    if (offset == 0)
                    {
                        ...
                        SPIFFS_CHECK_RES(res);
                        res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_IX | SPIFFS_OP_C_UPDT, fd->file_nbr,
                                         SPIFFS_PAGE_TO_PADDR(fs, cur_objix_pix), SPIFFS_CFG_LOG_PAGE_SZ(fs), fs->work);
                        SPIFFS_CHECK_RES(res);
                    }
                    else
                    {
                        // was a nonempty object, update to new page
                        res = spiffs_object_update_index_hdr(fs, fd, fd->obj_id, fd->objix_hdr_pix, fs->work, 0,
                                                             offset + written, &new_objix_hdr_page);
                        SPIFFS_CHECK_RES(res);
                        SPIFFS_DBG(
                            "append: %04x store new objix_hdr, %04x:%04x, "
                            "written %i\n",
                ...
                else
                {
                    ...
                    res = spiffs_object_update_index_hdr(fs, fd, fd->obj_id, fd->objix_hdr_pix, 0, 0, offset + written,
                                                         &new_objix_hdr_page);
                    SPIFFS_CHECK_RES(res);
                    SPIFFS_DBG(
                        "append: %04x store new size I %i in objix_hdr, "
                        "%04x:%04x, written %i\n",
                        fd->obj_id, offset + written, new_objix_hdr_page, 0, written);
                }
            ...
            else
            {
                spiffs_span_ix len_objix_spix =
                    SPIFFS_OBJ_IX_ENTRY_SPAN_IX(fs, (fd->size - 1) / SPIFFS_DATA_PAGE_SIZE(fs));
                ...
                if (written > 0 || cur_objix_spix > len_objix_spix)
                {
                    ...
                    res = spiffs_page_allocate_data(fs, fd->obj_id | SPIFFS_OBJ_ID_IX_FLAG, &p_hdr, 0, 0, 0, 1,
                                                    &cur_objix_pix);
                    SPIFFS_CHECK_RES(res);
                    ...
                    SPIFFS_DBG(
                        "append: %04x create objix page, %04x:%04x, written "
                        "%i\n",
                ...
                else
                {
                    ...
                    SPIFFS_DBG(
                        "append: %04x found object index at page %04x [fd size "
                        "%i]\n",
                        fd->obj_id, pix, fd->size);
                    res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_IX | SPIFFS_OP_C_READ, fd->file_nbr,
                                     SPIFFS_PAGE_TO_PADDR(fs, pix), SPIFFS_CFG_LOG_PAGE_SZ(fs), fs->work);
                    SPIFFS_CHECK_RES(res);
                    SPIFFS_VALIDATE_OBJIX(objix_hdr->p_hdr, fd->obj_id, cur_objix_spix);
                    cur_objix_pix = pix;
                }
        ...
        if (page_offs == 0)
        {
            ...
            res = spiffs_page_allocate_data(fs, fd->obj_id & ~SPIFFS_OBJ_ID_IX_FLAG, &p_hdr, &data[written], to_write,
                                            page_offs, 1, &data_page);
            SPIFFS_DBG(
                "append: %04x store new data page, %04x:%04x offset:%i, len "
                "%i, written %i\n",
        ...
        else
        {
            ...
            else
            {
                ...
                data_page = ((spiffs_page_ix *)((u8_t *)objix +
                                                sizeof(spiffs_page_object_ix)))[SPIFFS_OBJ_IX_ENTRY(fs, data_spix)];
            }

            res = spiffs_page_data_check(fs, fd, data_page, data_spix);
            SPIFFS_CHECK_RES(res);

            res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_DA | SPIFFS_OP_C_UPDT, fd->file_nbr,
                             SPIFFS_PAGE_TO_PADDR(fs, data_page) + sizeof(spiffs_page_header) + page_offs, to_write,
                             &data[written]);
            SPIFFS_DBG(
                "append: %04x store to existing data page, %04x:%04x "
                "offset:%i, len %i, written %i\n",
                fd->obj_id, data_page, data_spix, page_offs, to_write, written);
        }
        ...
        else
        {
            ...
            ((spiffs_page_ix *)((u8_t *)objix + sizeof(spiffs_page_object_ix)))[SPIFFS_OBJ_IX_ENTRY(fs, data_spix)] =
                data_page;
    ...
    if (cur_objix_spix != 0)
    {
        ...
        // write last modified object index page, unless object header index
        // page
        ...
        res2 = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_IX | SPIFFS_OP_C_UPDT, fd->file_nbr,
                          SPIFFS_PAGE_TO_PADDR(fs, cur_objix_pix), SPIFFS_CFG_LOG_PAGE_SZ(fs), fs->work);
        SPIFFS_CHECK_RES(res2);
        ...
        res2 = spiffs_object_update_index_hdr(fs, fd, fd->obj_id, fd->objix_hdr_pix, 0, 0, offset + written,
                                              &new_objix_hdr_page);
        SPIFFS_DBG(
            "append: %04x store new size II %i in objix_hdr, %04x:%04x, "
            "written %i, res %i\n",
            fd->obj_id, offset + written, new_objix_hdr_page, 0, written, res2);
        SPIFFS_CHECK_RES(res2);
    }
    else
    {
        ...
        if (offset == 0)
        {
            ...
            objix_hdr->size = offset + written;
            SPIFFS_DBG(
                "append: %04x store fresh objix_hdr page, %04x:%04x, written "
                "%i\n",
                fd->obj_id, cur_objix_pix, cur_objix_spix, written);
        ...
        else
        {
            ...
            res2 = spiffs_object_update_index_hdr(fs, fd, fd->obj_id, fd->objix_hdr_pix, fs->work, 0, offset + written,
                                                  &new_objix_hdr_page);
            SPIFFS_DBG(
                "append: %04x store modified objix_hdr page, %04x:%04x, "
                "written %i\n",
                fd->obj_id, new_objix_hdr_page, 0, written);
            SPIFFS_CHECK_RES(res2);
        }
...
s32_t spiffs_object_modify(spiffs_fd *fd, u32_t offset, u8_t *data, u32_t len)
{
    ...
    while (res == SPIFFS_OK && written < len)
    {
        ...
        if (cur_objix_spix != prev_objix_spix)
        {
            ...
            if (written > 0)
            {
                ...
                if (prev_objix_spix == 0)
                {
                    ...
                    res = spiffs_object_update_index_hdr(fs, fd, fd->obj_id, fd->objix_hdr_pix, fs->work, 0, 0,
                                                         &new_objix_hdr_pix);
                    SPIFFS_DBG(
                        "modify: store modified objix_hdr page, %04x:%04x, "
                        "written %i\n",
                ...
                else
                {
                    ...
                    res =
                        spiffs_page_move(fs, fd->file_nbr, (u8_t *)objix, fd->obj_id, 0, cur_objix_pix, &new_objix_pix);
                    SPIFFS_DBG(
                        "modify: store previous modified objix page, "
                        "%04x:%04x, written %i\n",
            ...
            else
            {
                ...
                {
                    res =
                        spiffs_obj_lu_find_id_and_span(fs, fd->obj_id | SPIFFS_OBJ_ID_IX_FLAG, cur_objix_spix, 0, &pix);
                    SPIFFS_CHECK_RES(res);
                }
        ...
        if (page_offs == 0 && to_write == SPIFFS_DATA_PAGE_SIZE(fs))
        {
            ...
            SPIFFS_DBG(
                "modify: store new data page, %04x:%04x offset:%i, len %i, "
                "written %i\n",
        ...
        else
        {
            ...
            res = spiffs_page_allocate_data(fs, fd->obj_id & ~SPIFFS_OBJ_ID_IX_FLAG, &p_hdr, 0, 0, 0, 0, &data_pix);
            if (res != SPIFFS_OK)
                break;
            ...
            {
                // before modification
                res = spiffs_phys_cpy(fs, fd->file_nbr, SPIFFS_PAGE_TO_PADDR(fs, data_pix) + sizeof(spiffs_page_header),
            ...
            p_hdr.flags &= ~SPIFFS_PH_FLAG_FINAL;
            res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_DA | SPIFFS_OP_C_UPDT, fd->file_nbr,
                             SPIFFS_PAGE_TO_PADDR(fs, data_pix) + offsetof(spiffs_page_header, flags), sizeof(u8_t),
                             (u8_t *)&p_hdr.flags);
            if (res != SPIFFS_OK)
                break;

            SPIFFS_DBG(
                "modify: store to existing data page, src:%04x, dst:%04x:%04x "
                "offset:%i, len %i, written %i\n",
    ...
    else
    {
        // wrote within object index header page
        res2 =
            spiffs_object_update_index_hdr(fs, fd, fd->obj_id, fd->objix_hdr_pix, fs->work, 0, 0, &new_objix_hdr_pix);
...
static s32_t spiffs_object_find_object_index_header_by_name_v(spiffs *fs, spiffs_obj_id obj_id, spiffs_block_ix bix,
                                                              int ix_entry, const void *user_const_p, void *user_var_p)
{
    (void)user_var_p;
    s32_t res;
    spiffs_page_object_ix_header objix_hdr;
    ...
    res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_LU2 | SPIFFS_OP_C_READ, 0, SPIFFS_PAGE_TO_PADDR(fs, pix),
                     sizeof(spiffs_page_object_ix_header), (u8_t *)&objix_hdr);
    SPIFFS_CHECK_RES(res);
                                                     ...
                                                     spiffs_page_ix *pix)
{
    ...
    res = spiffs_obj_lu_find_entry_visitor(fs, fs->cursor_block_ix, fs->cursor_obj_lu_entry, 0, 0,
                                           spiffs_object_find_object_index_header_by_name_v, name, 0, &bix, &entry);
...
// totally
s32_t spiffs_object_truncate(spiffs_fd *fd, u32_t new_size, u8_t remove)
{
    s32_t res = SPIFFS_OK;
    spiffs *fs = fd->fs;
    ...
    while (cur_size > new_size)
    {
        ...
        if (prev_objix_spix != cur_objix_spix)
        {
            if (prev_objix_spix != (spiffs_span_ix)-1)
            {
                ...
                if (prev_objix_spix > 0)
                {
                    ...
                    // to remove the file. If fully removing, we're not keeping
                    // consistency as good as when storing the header between
                    // chunks, would we be aborted. But when removing full
                    // files, a crammed system may otherwise report ERR_FULL a
                    // la windows. We cannot have that. Hence, take the risk -
                    // if aborted, a file check would free the lost pages and
                    // mend things as the file is marked as fully deleted in the
                    // beginning.
                    if (remove == 0)
                    {
                        SPIFFS_DBG(
                            "truncate: update objix hdr page %04x:%04x to size "
                            "%i\n",
                            fd->objix_hdr_pix, prev_objix_spix, cur_size);
                        res = spiffs_object_update_index_hdr(fs, fd, fd->obj_id, fd->objix_hdr_pix, 0, 0, cur_size,
                                                             &new_objix_hdr_pix);
                        SPIFFS_CHECK_RES(res);
                    }
                    fd->size = cur_size;
            ...
            SPIFFS_DBG("truncate: load objix page %04x:%04x for data spix:%04x\n", objix_pix, cur_objix_spix,
                       data_spix);
            res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_IX | SPIFFS_OP_C_READ, fd->file_nbr,
                             SPIFFS_PAGE_TO_PADDR(fs, objix_pix), SPIFFS_CFG_LOG_PAGE_SZ(fs), fs->work);
            SPIFFS_CHECK_RES(res);
            SPIFFS_VALIDATE_OBJIX(objix_hdr->p_hdr, fd->obj_id, cur_objix_spix);
        ...
        if (new_size == 0 || remove || cur_size - new_size >= SPIFFS_DATA_PAGE_SIZE(fs))
        {
            ...
            fd->offset = cur_size;
            SPIFFS_DBG(
                "truncate: delete data page %04x for data spix:%04x, "
                "cur_size:%i\n",
        ...
        else
        {
            ...
            SPIFFS_DBG(
                "truncate: delete %i bytes from data page %04x for data "
                "spix:%04x, cur_size:%i\n",
            ...
            res = spiffs_page_allocate_data(fs, fd->obj_id & ~SPIFFS_OBJ_ID_IX_FLAG, &p_hdr, 0, 0, 0, 0, &new_data_pix);
            if (res != SPIFFS_OK)
                break;
            res = spiffs_phys_cpy(fs, 0, SPIFFS_PAGE_TO_PADDR(fs, new_data_pix) + sizeof(spiffs_page_header),
            ...
            p_hdr.flags &= ~SPIFFS_PH_FLAG_FINAL;
            res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_DA | SPIFFS_OP_C_UPDT, fd->file_nbr,
                             SPIFFS_PAGE_TO_PADDR(fs, new_data_pix) + offsetof(spiffs_page_header, flags), sizeof(u8_t),
                             (u8_t *)&p_hdr.flags);
            if (res != SPIFFS_OK)
                break;
            ...
            if (cur_objix_spix == 0)
            {
                ...
                ((spiffs_page_ix *)((u8_t *)objix_hdr + sizeof(spiffs_page_object_ix_header)))[data_spix] =
                    new_data_pix;
                SPIFFS_DBG(
                    "truncate: wrote page %04x to objix_hdr entry %02x in "
                    "mem\n",
            ...
            {
                // update object index page
                ((spiffs_page_ix *)((u8_t *)objix +
                                    sizeof(spiffs_page_object_ix)))[SPIFFS_OBJ_IX_ENTRY(fs, data_spix)] = new_data_pix;
    ...
    if (cur_objix_spix == 0)
    {
        ...
        if (cur_size == 0)
        {
            ...
            else
            {
                ...
                res = spiffs_object_update_index_hdr(fs, fd, fd->obj_id, objix_pix, fs->work, 0, SPIFFS_UNDEFINED_LEN,
                                                     &new_objix_hdr_pix);
                SPIFFS_CHECK_RES(res);
            }
        }
        else
        {
            // update object index header page
            SPIFFS_DBG(
                "truncate: update object index header page with indices and "
                "size\n");
            res = spiffs_object_update_index_hdr(fs, fd, fd->obj_id, objix_pix, fs->work, 0, cur_size,
                                                 &new_objix_hdr_pix);
            SPIFFS_CHECK_RES(res);
        }
    }
    else
    {
        ...
        res = spiffs_object_update_index_hdr(fs, fd, fd->obj_id, fd->objix_hdr_pix, 0, 0, cur_size, &new_objix_hdr_pix);
        SPIFFS_CHECK_RES(res);
    }
    fd->size = cur_size;
...
s32_t spiffs_object_read(spiffs_fd *fd, u32_t offset, u32_t len, u8_t *dst)
{
    ...
    while (cur_offset < offset + len)
    {
        ...
        SPIFFS_CHECK_RES(res);
        res = _spiffs_rd(
            fs, SPIFFS_OP_T_OBJ_DA | SPIFFS_OP_C_READ, fd->file_nbr,
            SPIFFS_PAGE_TO_PADDR(fs, data_pix) + sizeof(spiffs_page_header) + (cur_offset % SPIFFS_DATA_PAGE_SIZE(fs)),
            len_to_read, dst);
        SPIFFS_CHECK_RES(res);
        dst += len_to_read;
                                                      ...
                                                      const void *user_const_p, void *user_var_p)
{
    ...
    if (id != SPIFFS_OBJ_ID_FREE && id != SPIFFS_OBJ_ID_DELETED && (id & SPIFFS_OBJ_ID_IX_FLAG))
    {
        ...
        res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_LU2 | SPIFFS_OP_C_READ, 0,
                         SPIFFS_OBJ_LOOKUP_ENTRY_TO_PADDR(fs, bix, ix_entry), sizeof(spiffs_page_object_ix_header),
                         (u8_t *)&objix_hdr);
             ...
             (SPIFFS_PH_FLAG_DELET)))
        {
            ...
            if (id >= state->min_obj_id && id <= state->max_obj_id)
            {
                ...
                // SPIFFS_DBG("free_obj_id: add ix %i for id %04x min:%04x
                // max%04x comp:%i\n", ix, id, state->min_obj_id,
...
// Scans thru all object lookup for object index header pages. If total possible
// number of object ids cannot fit into a work buffer, these are grouped. When a
// group containing free object ids is found, the object lu is again scanned for
// object ids within group and bitmasked. Finally, the bitmask is searched for a
// free id
    ...
    while (res == SPIFFS_OK && free_obj_id == SPIFFS_OBJ_ID_FREE)
    {
        ...
        else
        {
            ...
            if (state.compaction != 0)
            {
                ...
                SPIFFS_DBG(
                    "free_obj_id: COMP select index:%i min_count:%i min:%04x "
                    "max:%04x compact:%i\n",
            ...
            // log_page_size ids todo what if compaction is > 255 - then we
            // cannot fit it in a byte
            ...
            memset(fs->work, 0, SPIFFS_CFG_LOG_PAGE_SZ(fs));
            res = spiffs_obj_lu_find_entry_visitor(fs, 0, 0, 0, 0, spiffs_obj_lu_find_free_obj_id_compact_v, &state, 0,
                                                   0, 0);
            if (res == SPIFFS_VIS_END)
                res = SPIFFS_OK;
            SPIFFS_CHECK_RES(res);
