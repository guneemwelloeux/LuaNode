    ...
    spiffs* fs, u16_t max_free_pages)
{
    ...
    while (res == SPIFFS_OK && blocks--)
    {
        ...
        if (res == SPIFFS_OK && deleted_pages_in_block + free_pages_in_block == SPIFFS_PAGES_PER_BLOCK(fs) - SPIFFS_OBJ_LOOKUP_PAGES(fs) && free_pages_in_block <= max_free_pages)
        {
    ...
    u32_t len)
{
    ...
    } while (++tries < SPIFFS_GC_MAX_RUNS && (fs->free_blocks <= 2 || (s32_t)len > free_pages * (s32_t)SPIFFS_DATA_PAGE_SIZE(fs)));
    ...
    *block_candidates = cand_blocks;
    ...
    while (res == SPIFFS_OK && blocks--)
    {
        ...
        if (res == SPIFFS_OK && deleted_pages_in_block > 0)
        {
            ...
            s32_t score = deleted_pages_in_block * SPIFFS_GC_HEUR_W_DELET + used_pages_in_block * SPIFFS_GC_HEUR_W_USED + erase_age * (fs_crammed ? 0 : SPIFFS_GC_HEUR_W_ERASE_AGE);
            int cand_ix = 0;
...
s32_t spiffs_gc_clean(spiffs* fs, spiffs_block_ix bix)
{
    ...
    while (res == SPIFFS_OK && gc.state != FINISHED)
    {
        ...
        while (scan && res == SPIFFS_OK && obj_lookup_page < (int)SPIFFS_OBJ_LOOKUP_PAGES(fs))
        {
            ...
            // check each entry
            while (scan && res == SPIFFS_OK && cur_entry - entry_offset < entries_per_page && cur_entry < (int)(SPIFFS_PAGES_PER_BLOCK(fs) - SPIFFS_OBJ_LOOKUP_PAGES(fs)))
            {
                spiffs_obj_id obj_id = obj_lu_buf[cur_entry - entry_offset];
                ...
                {
                case FIND_OBJ_DATA:
                    if (obj_id != SPIFFS_OBJ_ID_DELETED && obj_id != SPIFFS_OBJ_ID_FREE && ((obj_id & SPIFFS_OBJ_ID_IX_FLAG) == 0))
                    {
                ...
                case MOVE_OBJ_DATA:
                    ...
                    break;
                case MOVE_OBJ_IX:
                    if (obj_id != SPIFFS_OBJ_ID_DELETED && obj_id != SPIFFS_OBJ_ID_FREE && (obj_id & SPIFFS_OBJ_ID_IX_FLAG))
                    {
