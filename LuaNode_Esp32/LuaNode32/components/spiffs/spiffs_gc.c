...
{
    s32_t res;
    s32_t free_pages = (SPIFFS_PAGES_PER_BLOCK(fs) - SPIFFS_OBJ_LOOKUP_PAGES(fs)) * (fs->block_count - 2) -
                       fs->stats_p_allocated - fs->stats_p_deleted;
    int tries = 0;

    if (fs->free_blocks > 3 && (s32_t)len < free_pages * (s32_t)SPIFFS_DATA_PAGE_SIZE(fs))
    {
        return SPIFFS_OK;
    }
    ...
    {
        SPIFFS_GC_DBG(
            "\ngc_check #%i: run gc free_blocks:%i pfree:%i pallo:%i pdele:%i "
            "[%i] len:%i of %i\n",
            tries, fs->free_blocks, free_pages, fs->stats_p_allocated, fs->stats_p_deleted,
            (free_pages + fs->stats_p_allocated + fs->stats_p_deleted), len, free_pages * SPIFFS_DATA_PAGE_SIZE(fs));
        ...
        free_pages = (SPIFFS_PAGES_PER_BLOCK(fs) - SPIFFS_OBJ_LOOKUP_PAGES(fs)) * (fs->block_count - 2) -
                     fs->stats_p_allocated - fs->stats_p_deleted;
        ...
        if (prev_free_pages <= 0 && prev_free_pages == free_pages)
        {
            ...
            break;
             ...
             (fs->free_blocks <= 2 || (s32_t)len > free_pages * (s32_t)SPIFFS_DATA_PAGE_SIZE(fs)));
    ...
    SPIFFS_GC_DBG(
        "gc_check: finished, %i dirty, blocks %i free, %i pages free, %i "
        "tries, res %i\n",
        fs->stats_p_allocated + fs->stats_p_deleted, fs->free_blocks, free_pages, tries, res);
...
s32_t spiffs_gc_erase_page_stats(spiffs *fs, spiffs_block_ix bix)
{
    ...
    while (res == SPIFFS_OK && obj_lookup_page < (int)SPIFFS_OBJ_LOOKUP_PAGES(fs))
    {
        int entry_offset = obj_lookup_page * entries_per_page;
        res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_LU | SPIFFS_OP_C_READ, 0,
                         bix * SPIFFS_CFG_LOG_BLOCK_SZ(fs) + SPIFFS_PAGE_TO_PADDR(fs, obj_lookup_page),
...
s32_t spiffs_gc_find_candidate(spiffs *fs, spiffs_block_ix **block_candidates, int *candidate_count, char fs_crammed)
{
    s32_t res = SPIFFS_OK;
    u32_t blocks = fs->block_count;
    spiffs_block_ix cur_block = 0;
    ...
    // (spiffs_block_ix)cand_bix/(s32_t)score
    int max_candidates =
        MIN(fs->block_count, (SPIFFS_CFG_LOG_PAGE_SZ(fs) - 8) / (sizeof(spiffs_block_ix) + sizeof(s32_t)));
    *candidate_count = 0;
    ...
    *block_candidates = cand_blocks;
    ...
    while (res == SPIFFS_OK && blocks--)
    {
        ...
        if (res == SPIFFS_OK && deleted_pages_in_block > 0)
        {
            ...
            res = _spiffs_rd(fs, SPIFFS_OP_C_READ | SPIFFS_OP_T_OBJ_LU2, 0, SPIFFS_ERASE_COUNT_PADDR(fs, cur_block),
...
//   for first found id, check spix and load corresponding object index page to
//   memory push object scan lookup entry index
//     rescan object lookup, find data pages with same id and referenced by same
//     object index move data page, update object index in memory when reached
//     end of lookup, store updated object index
//   pop object scan lookup entry index
//   repeat loop until end of object lookup
//   scan object lookup again for remaining object index pages, move to new page
//   in other block
//
    ...
    while (res == SPIFFS_OK && gc.state != FINISHED)
    {
        ...
        while (scan && res == SPIFFS_OK && obj_lookup_page < (int)SPIFFS_OBJ_LOOKUP_PAGES(fs))
        {
            ...
            res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_LU | SPIFFS_OP_C_READ, 0,
                             ...
                             SPIFFS_CFG_LOG_PAGE_SZ(fs), fs->lu_work);
            // check each entry
            while (scan && res == SPIFFS_OK && cur_entry - entry_offset < entries_per_page &&
                   cur_entry < (int)(SPIFFS_PAGES_PER_BLOCK(fs) - SPIFFS_OBJ_LOOKUP_PAGES(fs)))
            {
                spiffs_obj_id obj_id = obj_lu_buf[cur_entry - entry_offset];
                ...
                switch (gc.state)
                {
                    case FIND_OBJ_DATA:
                        ...
                        {
                            SPIFFS_GC_DBG(
                                "gc_clean: FIND_DATA state:%i - found obj id "
                                "%04x\n",
                                gc.state, obj_id);
                            gc.obj_id_found = 1;
                    ...
                    case MOVE_OBJ_DATA:
                        if (obj_id == gc.cur_obj_id)
                        {
                            ...
                            SPIFFS_CHECK_RES(res);
                            SPIFFS_GC_DBG(
                                "gc_clean: MOVE_DATA found data page %04x:%04x "
                                "@ %04x\n",
                                gc.cur_obj_id, p_hdr.span_ix, cur_pix);
                            if (SPIFFS_OBJ_IX_ENTRY_SPAN_IX(fs, p_hdr.span_ix) != gc.cur_objix_spix)
                            {
                                SPIFFS_GC_DBG(
                                    "gc_clean: MOVE_DATA no objix spix match, "
                                    "take in another run\n");
                            }
                            else
                            {
                                ...
                                if (p_hdr.flags & SPIFFS_PH_FLAG_DELET)
                                {
                                    ...
                                    SPIFFS_GC_DBG(
                                        "gc_clean: MOVE_DATA move objix "
                                        "%04x:%04x page %04x to %04x\n",
                                        gc.cur_obj_id, p_hdr.span_ix, cur_pix, new_data_pix);
                                    SPIFFS_CHECK_RES(res);
                                    // move wipes obj_lu, reload it
                                    res = _spiffs_rd(
                                        fs, SPIFFS_OP_T_OBJ_LU | SPIFFS_OP_C_READ, 0,
                                        bix * SPIFFS_CFG_LOG_BLOCK_SZ(fs) + SPIFFS_PAGE_TO_PADDR(fs, obj_lookup_page),
                                        SPIFFS_CFG_LOG_PAGE_SZ(fs), fs->lu_work);
                                    SPIFFS_CHECK_RES(res);
                                }
                                ...
                                {
                                    // page is deleted but not deleted in
                                    // lookup, scrap it
                                    SPIFFS_GC_DBG(
                                        "gc_clean: MOVE_DATA wipe objix "
                                        "%04x:%04x page %04x\n",
                                ...
                                if (gc.cur_objix_spix == 0)
                                {
                                    ...
                                    ((spiffs_page_ix *)((u8_t *)objix_hdr +
                                                        sizeof(spiffs_page_object_ix_header)))[p_hdr.span_ix] =
                                        new_data_pix;
                                    SPIFFS_GC_DBG(
                                        "gc_clean: MOVE_DATA wrote page %04x "
                                        "to objix_hdr entry %02x in mem\n",
                    ...
                    case MOVE_OBJ_IX:
                            ...
                            (obj_id & SPIFFS_OBJ_ID_IX_FLAG))
                        {
                            ...
                            if (p_hdr.flags & SPIFFS_PH_FLAG_DELET)
                            {
                                ...
                                SPIFFS_GC_DBG(
                                    "gc_clean: MOVE_OBJIX move objix %04x:%04x "
                                    "page %04x to %04x\n",
                            ...
                            else
                            {
                                ...
                                // scrap it
                                SPIFFS_GC_DBG(
                                    "gc_clean: MOVE_OBJIX wipe objix %04x:%04x "
                                    "page %04x\n",
                                    obj_id, p_hdr.span_ix, cur_pix);
                                res = spiffs_page_delete(fs, cur_pix);
                                if (res == SPIFFS_OK)
            ...
            case MOVE_OBJ_DATA:
            {
                ...
                if (gc.cur_objix_spix == 0)
                {
                    ...
                    res = spiffs_object_update_index_hdr(fs, 0, gc.cur_obj_id | SPIFFS_OBJ_ID_IX_FLAG, gc.cur_objix_pix,
                                                         fs->work, 0, 0, &new_objix_pix);
                    SPIFFS_GC_DBG(
                        "gc_clean: MOVE_DATA store modified objix_hdr page, "
                        "%04x:%04x\n",
                        new_objix_pix, 0);
                    SPIFFS_CHECK_RES(res);
                }
                else
                {
                    ...
                    res = spiffs_page_move(fs, 0, fs->work, gc.cur_obj_id | SPIFFS_OBJ_ID_IX_FLAG, 0, gc.cur_objix_pix,
                                           &new_objix_pix);
                    SPIFFS_GC_DBG(
                        "gc_clean: MOVE_DATA store modified objix page, "
                        "%04x:%04x\n",
                        new_objix_pix, objix->p_hdr.span_ix);
                    SPIFFS_CHECK_RES(res);
                    spiffs_cb_object_event(fs, 0, SPIFFS_EV_IX_UPD, gc.cur_obj_id, objix->p_hdr.span_ix, new_objix_pix,
                                           0);
                }
            }
            break;
