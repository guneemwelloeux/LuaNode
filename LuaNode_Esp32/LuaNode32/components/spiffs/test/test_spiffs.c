...
{
    int i;
    for (i = 0; i < size; i++) { area[addr + i] = *buf++; }
}

void area_read(u32_t addr, u8_t *buf, u32_t size)
{
    int i;
    for (i = 0; i < size; i++) { *buf++ = area[addr + i]; }
}
...
void dump_erase_counts(spiffs *fs)
{
    ...
    printf("  BLOCK     |\n");
    printf("   AGE COUNT|\n");
    for (bix = 0; bix < fs->block_count; bix++) { printf("----%3i ----|", bix); }
    printf("\n");
...
void fs_reset_specific(u32_t phys_addr, u32_t phys_size, u32_t phys_sector_size, u32_t log_block_size,
                       u32_t log_page_size)
{
...
void fs_reset() { fs_reset_specific(SPIFFS_PHYS_ADDR, SPIFFS_FLASH_SIZE, SECTOR_SIZE, LOG_BLOCK, LOG_PAGE); }

void set_flash_ops_log(int enable) { log_flash_ops = enable; }
...
u32_t get_flash_ops_log_read_bytes() { return bytes_rd; }

u32_t get_flash_ops_log_write_bytes() { return bytes_wr; }
...
void fs_set_validate_flashing(int i) { check_valid_flash = i; }
...
int read_and_verify_fd(spiffs_file fd, char *name)
{
    ...
    while (offs < s.size)
    {
        ...
        {
            printf(
                "  read_and_verify: could not read file %s offs:%i len:%i "
                "filelen:%i\n",
        ...
        for (i = 0; veri_ok && i < read_len; i++)
        {
            if (buf_d[i] != buf_v[i])
            {
                printf("file verification mismatch @ %i, %02x %c != %02x %c\n", offs + i, buf_d[i], buf_d[i], buf_v[i],
                       buf_v[i]);
                int j = MAX(0, i - 16);
                int k = MIN(sizeof(buf_d), i + 16);
                k = MIN(s.size - offs, k);
                int l;
                for (l = j; l < k; l++) { printf("%c", buf_d[l] > 31 ? buf_d[l] : '.'); }
                printf("\n");
                for (l = j; l < k; l++) { printf("%c", buf_v[l] > 31 ? buf_v[l] : '.'); }
                printf("\n");
                veri_ok = 0;
            }
        }
    ...
    while (run < max_runs)
    {
        ...
        for (i = 0; i < max_concurrent_files; i++)
        {
            ...
            if (tf->state == 0 && cur_config_ix < cfg_count)
            {
                ...
                if (dbg)
                    printf("   create new %s with cfg %i/%i, size %i\n", name, (1 + cur_config_ix), cfg_count, size);
