...
#define DELAY(s)                                 \
    {                                            \
        vTaskDelay(1000 * s / portTICK_RATE_MS); \
    }
...
{
    int i;
    gpio_num_t pins[] = {GPIO_CE, GPIO_RESET, GPIO_DC, GPIO_DIN, GPIO_CLK, GPIO_BL};
...
void clear_screen(void)
{
    ...
    for (t = 0; t < 6; t++)
    {
        for (k = 0; k < 84; k++) { lcd_write_byte(0x00, 1); }
    }
}
...
void lcd_write_letter(uint8_t row, uint8_t page, uint8_t c)
{
    ...
    for (i = 0; i < 8; i++) { lcd_write_byte(shuzi[c * 16 + i], 1); }

    lcd_set_xy(row * 8, page + 1);
    for (i = 8; i < 16; i++) { lcd_write_byte(shuzi[c * 16 + i], 1); }
}
