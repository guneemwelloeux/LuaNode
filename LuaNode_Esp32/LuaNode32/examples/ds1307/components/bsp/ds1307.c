...
static uint8_t intToBCD(uint8_t num) { return ((num / 10) << 4) | (num % 10); }
...
time_t readValue()
{
    ...
    int i;
    for (i = 0; i < 7; i++) { ESP_LOGD(tag, "%d: 0x%.2x", i, data[i]); }
    ...
    ESP_LOGI(tag, "time: %d.%d.%d, %d:%d:%d", tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min,
             tm.tm_sec);
    return readTime;
}
