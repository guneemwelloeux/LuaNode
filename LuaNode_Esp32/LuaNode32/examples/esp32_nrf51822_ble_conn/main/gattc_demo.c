...
static const char tar_char[] = { 0x9e, 0xca, 0xdc, 0x24, 0x0e, 0xe5, 0xa9, 0xe0, 0x93, 0xf3, 0xa3, 0xb5, 0x02, 0x00, 0x40, 0x6e };
    ...
    case ESP_GAP_BLE_SCAN_RESULT_EVT:
    {
        ...
        case ESP_GAP_SEARCH_INQ_RES_EVT:
            ...
            if (equ == 1 && connet == false)
            {
                connet = true;
                //char name[32] = {0};
                //sprintf(name, "==> address type: %d, dev name: %s", scan_result->scan_rst.ble_addr_type, adv_name);
                //ESP_LOGI(TAG, "%s", name);
    ...
    case ESP_GATTC_GET_CHAR_EVT:
    {
        ...
        if (p_data->get_char.status == 0)
        {
            ...
            else if (char_id->uuid.len == ESP_UUID_LEN_128)
            {
                ...
                /*if (strncmp((char *)char_id->uuid.uuid.uuid128, tar_char, 16) == 0) {
					task_run = true;
				}*/
            }
            else
...
static void led_task(void* arg)
{
    while (1)
    {
        ...
        if (task_run)
        {
            ...
            {
                uint8_t val[] = { 'o', 'n' };
                esp_err_t err = esp_ble_gattc_write_char(client_if,
                    client_conn,
                    &nrf51_service_id,
                    &char_id,
                    sizeof(val),
