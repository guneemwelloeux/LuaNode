    ...
    switch (event)
    {
        ...
        case ESP_GAP_BLE_ADV_STOP_COMPLETE_EVT:
            ...
            /*case ESP_GAP_BLE_UPDATE_CONN_PARAMS_EVT:
                 ESP_LOGI(GATTS_TAG, "update connetion params status = %d,
               min_int = %d, max_int = %d,conn_int = %d,latency = %d, timeout =
               %d", param->update_conn_params.status,
...
#else
            esp_err_t adv_ret = esp_ble_gap_config_adv_data(&test_adv_data);
            if (adv_ret)
            {
                ESP_LOGE(GATTS_TAG, "config adv data failed, error code = %x", adv_ret);
            }
#endif
            ...
            if (!param->write.is_prep)
            {
                ...
                if (gl_profile_tab[PROFILE_A_APP_ID].descr_handle == param->write.handle && param->write.len == 2)
                {
                    ...
                    if (value == 0x0001)
                    {
                        ...
                        uint8_t value[15];
                        for (int i = 0; i < sizeof(value); ++i) { value[i] = i % 0xff; }
                        // the size of value[] need less than MTU size
                        esp_ble_gatts_send_indicate(gatts_if, param->write.conn_id,
                                                    gl_profile_tab[PROFILE_A_APP_ID].char_handle, sizeof(value), value,
                                                    false);
                    }
            ...
            if (!param->write.is_prep)
            {
                ...
                if (gl_profile_tab[PROFILE_B_APP_ID].descr_handle == param->write.handle && param->write.len == 2)
                {
                    ...
                    if (value == 0x0001)
                    {
                        ...
                        // the size of value[] need less than MTU size
                        esp_ble_gatts_send_indicate(gatts_if, param->write.conn_id,
                                                    gl_profile_tab[PROFILE_B_APP_ID].char_handle, sizeof(value), value,
                                                    false);
                    }
                    else if (value == 0x0000)
                    {
